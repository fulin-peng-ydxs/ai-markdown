...
- `WorkspaceSelectionOutcome`：`cancelled`、`already_open`、`ready`、`confirmation_required`。
...
- `cargo test --locked --manifest-path src-tauri/Cargo.toml`：43 个单元测试通过。
...
- 覆盖：取消零副作用、父目录确认、扩展名和链接拒绝、一次性消费、令牌缺失/过期/上限、链接换靶、稳定 ID、同根识别、打开失败释放、最近记录提交更新、路径失效复检与 POSIX 只读提示。
