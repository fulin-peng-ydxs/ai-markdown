...
- 状态：已完成。
- 本任务不创建 P1/P2 页面，不启用“打开”菜单，不创建工作区窗口，不扫描文件树；T11 已接入授权后的窗口协调与状态提交，T12/T13 可见消费者落地前仍不得把命令存在表述为完整打开流程。
...
- 授权成功只登记进程内 `WorkspaceRegistry`。T11 已把最近记录与根窗口会话收敛到同一次原子状态更新：窗口打开/绑定成功才提交，创建或状态写入失败时释放未使用授权；最近记录按最后打开时间排序并限制为 100 条。
...
- 菜单“打开文件夹/打开 Markdown”继续禁用：T11 已提供窗口协调命令，但父目录确认、目录决策和持久错误反馈仍没有可见页面消费者；T12/T13 接入完整链路后再同步启用。
...
- 没有页面、打开菜单点击、最近列表和重新授权 UI 验证；T11 的窗口创建/会话核心只在 mock runtime 与临时根验证，真实可见消费者属于 T12/T13，不外推为完整打开流程通过。
