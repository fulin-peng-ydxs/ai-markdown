...
- 前端镜像：`src/services/desktop/contracts.ts`，字段使用 camelCase，枚举值使用 snake_case；Rust 单测逐模型比较序列化字段集合，并全量比较错误码与 `DESKTOP_ERROR_CODES`，任一侧单独变更都会失败。T2 建立了 14 个基础错误码，T3 新增 4 个窗口错误码，T4 新增 6 个状态仓储错误码，均受同一 parity 测试约束。
