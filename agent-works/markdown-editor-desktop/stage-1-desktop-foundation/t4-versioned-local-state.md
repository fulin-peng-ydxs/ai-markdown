...
- T5 复审回归暴露状态测试临时目录仅用时间戳/进程号时可能在并发或快速重跑中碰撞；测试夹具已追加进程内原子序号，消除同名文件/目录互相污染，不改变生产状态路径。
...
- POSIX 临时文件内容在替换前执行 `sync_all`，但替换后没有再 `fsync` 父目录；极端断电时辅助状态的目录项耐久性不作承诺。该文件只保存最近记录/根会话，不承载 Markdown，当前接受此取舍。
- `.corrupt-{timestamp}` 备份当前没有自动修剪上限；正常路径只在检测到损坏/超限状态时生成。后续状态维护任务应增加有界保留策略，不能无限累积。
- T16 必须在 Windows runner 实际编译 `MoveFileExW` 分支，并覆盖目标存在、占用和替换失败；macOS 编译与源码核对不能替代该证据。
