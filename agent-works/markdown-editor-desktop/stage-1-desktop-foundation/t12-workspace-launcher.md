...
- 复核整改将打开快捷键收敛为“原生菜单加速键 → 菜单事件 → 页面动作”单链路，并以同步 ref 阻止双事件或快速双击并发打开多个系统选择器；菜单订阅失败不再覆盖真实状态仓储错误。
- 批量恢复在循环内只更新逐项状态，整批结束后统一读取一次快照；仍待恢复的失败项保留原状态和说明，不因刷新退回“等待”。打开按钮 hover 改为全局 `paper-hover` 语义 token，不再使用页面私有色值。
...
- React：`pnpm test:ui` 17/17，通过过滤/清除、移除文案、单文件范围确认、原生菜单映射、选择器防重入、批量恢复单次快照、菜单订阅错误隔离、四类窗口错误、Esc、关闭门禁和焦点返回。
