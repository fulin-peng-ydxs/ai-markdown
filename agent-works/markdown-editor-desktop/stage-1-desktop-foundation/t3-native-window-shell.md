...
- T11 必须把静态启动标签 `main` 与动态 `launcher-N` 收敛为统一、可迁移的稳定标签体系，同时迁移 capability 与根会话引用；T3 不提前引入工作区标签语义。
- T11 将四类窗口错误作为稳定协调结果交给调用方，T12 以持久错误面板或对话框承接重试/取消；T3 在尚无页面消费者时只写脱敏 stderr，不视为最终用户反馈闭环。
- T5/T11 在真实工作区与活动文件状态建立后消费 `document_title`；禁用菜单项只有在对应处理器和错误反馈同一任务落地后才能启用其快捷键。
