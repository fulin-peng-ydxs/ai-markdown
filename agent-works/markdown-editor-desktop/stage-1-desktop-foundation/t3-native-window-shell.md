...
- 权限：T3 当时仅允许 `main` 与 `launcher-*` 使用 `core:default`；T11 已把该历史标签族迁移为统一的 `plainroot-window-*`。没有新增文件、Shell、网络、Store 或数据库权限。
...
- T3 的动态 launcher 曾按 `launcher-N` 分配；T11 已统一为 `plainroot-window-N`，并保留“不覆盖已存在或待恢复标签”的约束。
...
- `src-tauri/capabilities/default.json`：T11 后只允许 `plainroot-window-*`；旧 `main`/`launcher-*` 根会话在启动时迁移，不继续保留 capability。回滚标签配置不涉及 Markdown，但必须同步状态迁移和窗口创建规则。
...
- T11 已把静态启动标签 `main` 与动态 `launcher-N` 收敛为 `plainroot-window-N`，同步 capability，并迁移历史根会话引用；稳定 workspaceId 通过会话映射到不可变原生标签，避免当前窗口替换目录时重建窗口。
- T11 已将四类窗口错误接入可由页面调用的类型化命令；T12/T13 仍须用持久错误面板或对话框承接重试/取消，当前菜单日志不视为最终用户反馈闭环。
- `document_title` 已具备工作区/文件标题契约，但活动文档消费者仍待 P1 接入；禁用菜单项只有在对应处理器和错误反馈同一任务落地后才能启用其快捷键。
