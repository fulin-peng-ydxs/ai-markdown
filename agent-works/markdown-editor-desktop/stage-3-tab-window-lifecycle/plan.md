...
>
> 计划状态：执行中（T35～T42 已完成；T43～T46 未实施）
>
...
- `WorkspaceWorkbench` 已消费唯一 `WorkspaceTabManager`，页面上的 `documentState`、`editorHandleRef` 和 `saveControllerRef` 只是活动 runtime 投影；T38 已接入可见页签层，T39 已接入最近关闭重开、当前会话元数据写入和路径影响预分析，T40 已接入全页签两阶段结算、批量关闭和磁盘成功后的多 runtime 原子重映射，T42 已消费既有非空会话并恢复轻量页签集合、活动项和视图元数据。
...
| R11 | 页签加载、保存、冲突、缺失、恢复和批量结算状态真实可见 | 进行中 | 部分覆盖 | T38～T46 | T42 已区分 unloaded、ready 和逐项 missing/permission/error，提供跳过/重试且不阻断其他页签；完整阶段状态矩阵仍待 T44～T46 |
| R12 | 阶段 7 实现工作区全文搜索 | 跳过 | 未覆盖 | - | 搜索菜单继续禁用 |
| R13 | 完整纳入：真实页签、唯一打开、排序、批量关闭、溢出、最近关闭、独立会话和恢复 | 进行中 | 一致 | T35～T46 | T42 已在既有模型、仓储和 runtime 上完成启动恢复、活动项回退、惰性加载与失败隔离；原生命令、真实重启 E2E 和阶段验收仍待后续 |
...
| 必须实现 | R11 | 操作与文件状态反馈 | 实现页签、结算、恢复和持久化新增状态 | T38～T46 | 进行中 | 部分覆盖 | T42 已提供 unloaded、逐项失败、跳过和重试反馈；完整验收仍属阶段 4 |
| 必须实现 | R12 | 工作区全文搜索 | 暂不纳入 | - | 跳过 | 未覆盖 | 阶段 7 实现；页签列表不冒充搜索 |
| 必须实现 | R13 | 多文档页签与最近关闭文档 | 完整实现真实页签、排序、批量关闭、最近关闭和会话恢复 | T35～T46 | 进行中 | 一致 | T42 已完成已有会话启动恢复、活动项回退、惰性加载、失败隔离和后续元数据写入接管；原生命令与阶段验收待 T43～T46 |
