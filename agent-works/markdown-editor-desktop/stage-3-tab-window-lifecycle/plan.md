...
- 2026-07-26 已在 T40 当前本地状态重新执行门禁：Node 独立回归 30/30、Vitest 31 个文件 247/247、Rust 200 项通过且 1 项手动性能探针忽略、类型检查、生产构建、Rust fmt/全 feature Clippy 和许可证 727/511/0 通过；T40 页签专项为 61/61。T39 的 macOS Tauri/WebKit 桌面套件 10/10 仍是最近一次真实桌面证据，T40 本轮尚未重跑桌面 E2E。各任务留痕中的测试数字只是当时的历史快照。最新远端证据仍为 GitHub Actions run `30082725332` 在 macOS/Windows 通过第二阶段 9 条套件，尚未覆盖第三阶段本地提交。
...
- 实际落地情况：已先新增并复核 `prototypes/tab-settlement-dialog.html`：1100×760 和 560×720 两档均无根级横向溢出，固定标题/动作区与独立滚动清单成立；dirty/conflict/readonly 等混合项未解决时最终动作禁用，放弃修改须二次确认，全部解决后才允许提交。
  ...
  - `WorkspaceTabManager` 新增全目标结算、单 revision 批量关闭和路径/runtime 原子重映射。所有目标在移除前先完成 incarnation 复核；任一目标阻塞或陈旧时不移除任何页签。批次取消只保留页签，不回滚已经真实成功的磁盘保存。
  ...
  - 前端专项 61/61、全量 Vitest 247/247、Node 独立回归 30/30、Rust 200 项通过且 1 项手动探针忽略、类型检查、生产构建、Rust fmt/全 feature Clippy 和许可证 727/511/0 均通过。T40 未修改 Rust、capability、依赖、原生菜单或产品配置；本轮未重跑桌面 E2E、Windows 或远端 CI，不能外推为 T40 双平台桌面验收。完整证据见 `t40-tab-settlement.md`。
...
| 目录操作影响多个打开文档 | 路径/recovery/图片链接漂移 | T40 已在操作前结算集合、磁盘成功后批量重映射；图片链接写回失败保持 dirty/真实提示，T45 再做桌面故障回归 |
