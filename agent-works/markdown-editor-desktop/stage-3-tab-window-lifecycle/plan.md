...
>
> 计划状态：执行中（T35～T43 已完成；T44～T46 未实施）
>
...
- 2026-07-27 已在 T43 当前本地状态重新执行门禁：Node 独立回归 30/30、Vitest 32 个文件 267/267、Rust 203 项通过且 1 项手动性能探针忽略、类型检查、生产构建、Rust fmt、全 feature Clippy、全 feature Rust 测试和许可证 727/511/0 通过；页签专项为 70/70，macOS Tauri/WebKit 既有 11/11 真桌面回归在当前代码上通过。T43 新增测试覆盖原生页签动作 parity、按聚焦窗口隔离的菜单启用态、既有 manager/共享菜单/统一结算路由，以及原生请求打开“所有页签”后的焦点。独立 macOS 原生走查确认“页签”菜单、启用态、点击切换、`Cmd+W` 关闭和 `Cmd+Shift+T` 重开真实生效；Computer Use 未能用 `Ctrl+Tab` 注入命中原生加速键，因此该组合键和 Windows 原生菜单仍由 T45 验证，不写成已通过。各任务留痕中的测试数字只是当时的历史快照。最新远端证据仍为 GitHub Actions run `30082725332` 在 macOS/Windows 通过第二阶段 9 条套件，尚未覆盖第三阶段本地提交。
...
| R2 | 文件树打开、重命名、移动、删除同步作用于全部相关页签 | 进行中 | 部分覆盖 | T37、T39～T40、T44～T45 | T40 已解除 T39 的临时阻断：命中打开页签时先安全结算，磁盘成功后一次提交全部 runtime 路径/身份和树状态；磁盘失败不改页签。阶段整体回归仍待 T44～T45 |
| R3 | 每页签独立消费现有排版编辑与统一 history | 进行中 | 部分覆盖 | T35、T37、T39、T44～T45 | T37 已完成每页签独立 session/history 与活动 editor 单挂载，T38 已提供可见切换入口；后续整体回归未完成 |
| R4 | 阶段 6 实现大纲；本阶段只保证活动页签单一来源可供未来消费 | 跳过 | 未覆盖 | - | 确认无假大纲入口 |
| R5 | 将保存、恢复、冲突和关闭保护扩展为全页签结算 | 进行中 | 部分覆盖 | T37、T40～T46 | T40 已完成不可变批次和一次性提交，T41 已让窗口替换、关闭和退出消费同一批次，T42 恢复活动页签后继续复用第二阶段恢复快照链；阶段回归仍待 T44～T46 |
| R6 | 保持每页签图片链路；目录/文件移动时处理全部相关打开文档的链接和状态 | 进行中 | 部分覆盖 | T37、T39～T40、T44～T45 | T40 已在路径提交后对全部受影响已加载页签重新计算内联图片链接并进入各自保存链；写回失败时保留 dirty 页签并明确提示，整体回归仍待 T44～T45 |
...
| R11 | 页签加载、保存、冲突、缺失、恢复和批量结算状态真实可见 | 进行中 | 部分覆盖 | T38～T46 | T43 已让原生菜单启用态消费聚焦窗口的真实页签数量、活动位置、最近关闭数量和 busy 状态；完整阶段状态矩阵仍待 T44～T46 |
| R12 | 阶段 7 实现工作区全文搜索 | 跳过 | 未覆盖 | - | 搜索菜单继续禁用 |
| R13 | 完整纳入：真实页签、唯一打开、排序、批量关闭、溢出、最近关闭、独立会话和恢复 | 进行中 | 一致 | T35～T46 | T43 已把切换、关闭、重开、批量关闭和页签列表接入原生菜单，全部复用既有 manager/menu/settlement；真实重启 E2E 和阶段验收仍待 T45～T46 |
...
| 必须实现 | R2 | 文件与目录管理 | 文件树入口、CRUD 与全部相关页签联动 | T37、T39～T40、T44～T45 | 进行中 | 部分覆盖 | T40 已用全页签结算替代临时阻断，并在磁盘成功后批量提交路径/runtime 或页签移除；整体回归待 T44～T45 |
| 必须实现 | R3 | 所见即所得 Markdown 编辑 | 每页签独立消费既有 DocumentSession/editor | T35、T37、T39、T44～T45 | 进行中 | 部分覆盖 | T37 已完成 P1 每页签独立 session/history 与活动 editor 单挂载，T38 已提供可见切换入口；阶段验收未完成 |
| 必须实现 | R4 | 当前文档大纲 | 暂不纳入 | - | 跳过 | 未覆盖 | 阶段 6 实现；活动页签只预留单一来源 |
| 必须实现 | R5 | 自动保存、恢复和冲突 | 把单文档保护扩展为关闭、替换、退出的全页签结算 | T37、T40～T46 | 进行中 | 部分覆盖 | T42 已让恢复活动页签继续消费既有恢复快照链；整体阶段验收待 T44～T46 |
| 必须实现 | R6 | 图片粘贴、拖放与资源管理 | 多页签路径/图片链接随文件树操作一致变更 | T37、T39～T40、T44～T45 | 进行中 | 部分覆盖 | T40 已对全部受影响已加载页签重算内联图片链接并进入各自保存链，失败保持 dirty/真实提示；整体回归待 T44～T45 |
...
| 必须实现 | R11 | 操作与文件状态反馈 | 实现页签、结算、恢复和持久化新增状态 | T38～T46 | 进行中 | 部分覆盖 | T43 已让原生菜单启用态消费聚焦窗口真实页签状态；完整验收仍属阶段 4 |
| 必须实现 | R12 | 工作区全文搜索 | 暂不纳入 | - | 跳过 | 未覆盖 | 阶段 7 实现；页签列表不冒充搜索 |
| 必须实现 | R13 | 多文档页签与最近关闭文档 | 完整实现真实页签、排序、批量关闭、最近关闭和会话恢复 | T35～T46 | 进行中 | 一致 | T43 已让原生命令复用 manager、共享菜单和统一结算；真实重启/多窗口 E2E 与阶段验收待 T45～T46 |
