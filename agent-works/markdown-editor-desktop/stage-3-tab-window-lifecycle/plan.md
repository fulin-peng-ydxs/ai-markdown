...
>
> 计划状态：已完成（T35～T46 全部完成；代码 run `30322298422` 已覆盖提交 `9ba28d1` 的全部 T45 实现，收口文档 run `30323620642` 已覆盖其上的纯文档提交 `a0268eb`，两次均为 macOS/Windows 双绿与成对 artifact；T46 整体复核、专项架构和阶段验收已落地）
>
...
- 当前架构：`architecture/desktop-foundation.md`、`architecture/markdown-document-editing.md`、`architecture/tab-window-lifecycle.md`。
...
- 2026-07-28 已在 T45 当前本地状态以 `pnpm verify:non-desktop` 从头执行统一门禁：Node 独立回归 30/30、Vitest 33 个文件 269/269、Rust no-default/all-features 均 207 项通过且 1 项手动性能探针忽略、类型检查、生产构建、Rust fmt、全 target/all-features Clippy 和许可证 727/511/0 通过。macOS Tauri/WebKit 由四个隔离桌面进程完成 15/15：12 条主链、1 条真实系统页签组合键、1 条重启前会话种子和 1 条重启后逐项失败隔离。macOS 下一/上一页签使用原生菜单 `Cmd+Option+Right/Left`；Windows `Ctrl+PageDown/PageUp` 由聚焦 WebView 平台适配消费并委托唯一命令处理器。代码 run `30322298422` 随后在提交 `9ba28d1` 上让 macOS/Windows 同时通过相同门禁、四段桌面链、生产构建和 artifact 上传；收口文档 run `30323620642` 又在纯文档提交 `a0268eb` 上重复通过。
...
- `WorkspaceWorkbench` 已消费唯一 `WorkspaceTabManager`，页面上的 `documentState`、`editorHandleRef` 和 `saveControllerRef` 只是活动 runtime 投影；T38 已接入可见页签层，T39 已接入最近关闭重开、当前会话元数据写入和路径影响预分析，T40 已接入全页签两阶段结算、批量关闭和磁盘成功后的多 runtime 原子重映射，T42 已消费既有非空会话并恢复轻量页签集合、活动项和视图元数据，T43 已让聚焦窗口的原生页签命令委托同一 manager、共享菜单和结算链。
...
- 新增页签上下文菜单、最近关闭列表、全页签结算弹层和打开方式设置弹层没有完整上游 HTML 原型；其流程由 R5/R13/R14/R30、7.1.6、7.1.7、7.2.3 明确定义。普通菜单与少字段偏好弹层直接复用现有区域、`AppDialog` 和 DESIGN 规范；T40 已先新增 `prototypes/tab-settlement-dialog.html`，按 1100×760 与 560×720 两档复核混合状态、滚动、明确动词、取消和最终提交门禁后实现生产组件；T41 已落地打开方式设置弹层，T43 只复用既有页签菜单入口，不新增页面或弹层。
...
| --- | --- | --- | --- | --- | --- |
| R1 | 在现有 Windows/macOS 壳内增加真实多页签和会话恢复，不重做安装/授权 | 已完成 | 部分覆盖 | T41～T46 | T45 run `30322298422` 已在 macOS/Windows 验证真实多页签、重启恢复与单项隔离；T46 阶段复核通过 |
| R2 | 文件树打开、重命名、移动、删除同步作用于全部相关页签 | 已完成 | 部分覆盖 | T37、T39～T40、T44～T45 | T40 已在磁盘成功后一次提交全部 runtime 路径/身份和树状态；T45 双平台主链已回归真实改名、移动、删除及图片链接写回 |
| R3 | 每页签独立消费现有排版编辑与统一 history | 已完成 | 部分覆盖 | T35、T37、T39、T44～T45 | T37 已完成每页签独立 session/history 与活动 editor 单挂载，T45 双平台桌面链已回归多页签编辑与单 adapter |
| R4 | 阶段 6 实现大纲；本阶段只保证活动页签单一来源可供未来消费 | 跳过 | 未覆盖 | - | 确认无假大纲入口 |
| R5 | 将保存、恢复、冲突和关闭保护扩展为全页签结算 | 已完成 | 部分覆盖 | T37、T40～T46 | T40～T42 已闭合全页签结算、窗口事务和恢复；T45 双平台验证真实替换拒绝、跨进程恢复和既有保存/冲突主链 |
| R6 | 保持每页签图片链路；目录/文件移动时处理全部相关打开文档的链接和状态 | 已完成 | 部分覆盖 | T37、T39～T40、T44～T45 | T40 已在路径提交后对全部受影响已加载页签重算内联图片链接并进入各自保存链；T45 双平台主链已回归真实磁盘移动与图片链接写回 |
...
| R10 | 每页签独立保留排版/源码模式和源码选择 | 已完成 | 部分覆盖 | T35、T37、T42、T44～T45 | T42 已恢复各页签模式、选择和锚点元数据并惰性加载真实内容；T45 双平台跨进程恢复通过 |
| R11 | 页签加载、保存、冲突、缺失、恢复和批量结算状态真实可见 | 已完成 | 部分覆盖 | T38～T46 | T43/T44 已固化聚焦窗口菜单与非桌面状态契约；T45 双平台验证桌面矩阵与恢复缺失项反馈，完整验收仍属阶段 4 |
| R12 | 阶段 7 实现工作区全文搜索 | 跳过 | 未覆盖 | - | 搜索菜单继续禁用 |
| R13 | 完整纳入：真实页签、唯一打开、排序、批量关闭、溢出、最近关闭、独立会话和恢复 | 已完成 | 一致 | T35～T46 | T45 已在 macOS/Windows 通过 15 条真实桌面链，含平台页签组合键、替换拒绝、跨进程恢复与成对 artifact；T46 阶段验收通过 |
| R14 | 纳入页签会话、打开偏好、全页签替换保护和独立窗口恢复；位置/尺寸/三栏布局留阶段 4 | 已完成 | 部分覆盖 | T36、T40～T46 | T42 已消费根会话引用并恢复独立窗口页签集合，T45 双平台验证单窗口跨进程恢复、缺失项隔离和当前窗口替换拒绝；真实多窗口整组退出/恢复与阶段 4 布局仍未完成 |
| R15 | 阶段 5 实现主题工作室 | 跳过 | 未覆盖 | - | P3 不注册 |
| R30 | 纳入页签切换/关闭、重新打开、关闭窗口和弹层焦点子集 | 已完成 | 部分覆盖 | T38～T46 | T45 已用 macOS `Cmd+Option+Right/Left` 与 Windows `Ctrl+PageDown/PageUp` 真实系统输入验证页签双向切换；本阶段子集验收通过 |
| R31 | 纳入 tablist/menu/dialog 的语义、焦点、非颜色状态和减少动态效果子集 | 已完成 | 部分覆盖 | T38、T40、T43～T46 | T43 复用共享 `TabMenu` 打开“所有页签”并恢复首项焦点，原生菜单无可执行目标时禁用；本阶段子集验收通过，完整对比度、系统偏好与阶段 4 矩阵仍待后续 |
...
- 当前窗口替换顺序：取得选择提案 → 全页签结算 → 尽力持久化旧页签会话 → Rust 提交窗口根替换 → 卸载旧 manager/watch → 加载或恢复新工作区。正文结算、授权或 Rust 提交失败继续阻断；内容无关会话元数据写失败只明确提示下次可能恢复到较早状态，不得把已经安全的用户困在窗口内，也不得留下第二个可写映射。
- 关闭窗口：先结算全部页签并尽力持久化最后会话，再让现有 Rust 协调器关闭；取消保持窗口。页签会话元数据是辅助恢复信息，其失败不改变正文内容安全结论。
...
- `WorkspaceTabBar`、`TabOverflowMenu`、`TabContextMenu`、`TabSettlementDialog`、`WorkspaceOpenDecisionDialog`、`WorkspaceOpenPreferenceDialog` 和 `WorkspaceTabManager` 形成稳定职责后登记到 `DESIGN.md`。
...
| P1 页签溢出 | 新增 `TabOverflowMenu`，复用 `focusContainment` 的可聚焦元素规则和现有按钮样式 | 新增可访问菜单，不新增抽屉/页面 | 输入为全部页签、最近关闭、可用动作和触发器引用 | 登记菜单焦点、Esc、返回和最近关闭失败反馈契约 | P1 大量页签与最近关闭真实消费；50 项、键盘、焦点及桌面重开测试 | T38～T39、T43～T45 |
...
| P1/P2 工作区打开方式 | 新增 `WorkspaceOpenDecisionDialog`、`WorkspaceOpenPreferenceDialog`，复用 `AppDialog` 和全局按钮 | 新增共享位置决策与单一偏好 dialog，不扩张成通用设置页 | `workspaceOpenDisposition` 默认/保存/读取/消费/重置；“记住选择”、P2 按钮和原生设置菜单注册 | 已登记配置字段、P1/P2 双入口、失败反馈和回退职责 | P1/P2/menu 真实消费；三值、记住选择、重置、损坏/写失败回退测试 | T41、T43～T45 |
| P1/P2 页签会话能力 | 新增 `WorkspaceTabManager`，复用 `DocumentSession`、`DocumentSaveController`、recovery/assets gateway | 新增非视觉运行时状态能力 | 不注册页面；通过 manager provider/props 供 P1，P2 恢复后创建 | DESIGN 运行时复用清单登记稳定职责；架构文档记录数据边界 | P1 活动会话、最近关闭与启动惰性恢复真实消费；manager、P1/P2 组件和后续真实重启 E2E | T35、T37、T39、T42、T44～T46 |
| P1 页签会话持久化 | 新增 `WorkspaceTabSessionPersistence`，复用 T36 Rust window-session gateway | 非视觉防抖/CAS 调度能力 | 输入 manager 快照、workspaceId 与错误回调；不注册页面或配置 | DESIGN 登记只保存轻量元数据；既有会话先冻结，恢复消费后显式接管写入 | P1 顺序/活动/最近项写入与启动恢复解冻；投影、冲突、组件和桌面 IPC 测试 | T39、T42、T44～T45 |
| P1 文件操作影响预检 | 新增纯 `tabPathImpact`，复用 `workspacePath` 与 `workspaceAssetPath` | 非视觉路径/引用分析能力 | 输入 manager 快照和 rename/move/delete 意图；不执行磁盘写入 | DESIGN 登记纯分析与 T40 提交边界 | P1 文件操作真实消费；多页签、目录和图片链接测试 | T39～T40、T44～T45 |

## 5. 需求与开发内容映射和开发状态

| 范围类别 | 需求编号 | 需求内容/来源 | 计划开发内容 | 对应任务 | 状态 | 偏差判断 | 说明 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| 必须实现 | R1 | Windows/macOS 本地优先桌面应用 | 在既有桌面壳内接入多页签、窗口会话和双平台回归 | T41～T46 | 已完成 | 部分覆盖 | T45 run `30322298422` 已在 macOS/Windows 验证真实多页签、重启恢复与单项隔离；T46 阶段复核通过 |
| 必须实现 | R2 | 文件与目录管理 | 文件树入口、CRUD 与全部相关页签联动 | T37、T39～T40、T44～T45 | 已完成 | 部分覆盖 | T40 已在磁盘成功后提交路径/runtime 或页签移除；T45 双平台主链已回归真实改名、移动、删除与图片链接写回 |
| 必须实现 | R3 | 所见即所得 Markdown 编辑 | 每页签独立消费既有 DocumentSession/editor | T35、T37、T39、T44～T45 | 已完成 | 部分覆盖 | 独立 session/history 与活动 editor 单挂载已纳入统一门禁；T45 双平台回归真实多页签编辑与单 adapter |
| 必须实现 | R4 | 当前文档大纲 | 暂不纳入 | - | 跳过 | 未覆盖 | 阶段 6 实现；活动页签只预留单一来源 |
| 必须实现 | R5 | 自动保存、恢复和冲突 | 把单文档保护扩展为关闭、替换、退出的全页签结算 | T37、T40～T46 | 已完成 | 部分覆盖 | 恢复与全页签结算已通过非桌面故障矩阵；T45 双平台验证真实替换拒绝、跨进程恢复和既有保存/冲突主链 |
| 必须实现 | R6 | 图片粘贴、拖放与资源管理 | 多页签路径/图片链接随文件树操作一致变更 | T37、T39～T40、T44～T45 | 已完成 | 部分覆盖 | 全部受影响已加载页签的链接重算与失败保持 dirty 已纳入门禁；T45 双平台回归真实磁盘移动和图片链接写回 |
...
| 必须实现 | R10 | Markdown 源码模式 | 每页签独立保留 visual/source 模式、选择与锚点 | T35、T37、T42、T44～T45 | 已完成 | 部分覆盖 | T42 已恢复模式、选择与锚点元数据并惰性加载正文；T45 双平台跨进程恢复通过 |
| 必须实现 | R11 | 操作与文件状态反馈 | 实现页签、结算、恢复和持久化新增状态 | T38～T46 | 已完成 | 部分覆盖 | 页签/结算/恢复/菜单状态已纳入非桌面门禁；T45 双平台验证桌面矩阵和恢复缺失项反馈，完整验收仍属阶段 4 |
| 必须实现 | R12 | 工作区全文搜索 | 暂不纳入 | - | 跳过 | 未覆盖 | 阶段 7 实现；页签列表不冒充搜索 |
| 必须实现 | R13 | 多文档页签与最近关闭文档 | 完整实现真实页签、排序、批量关闭、最近关闭和会话恢复 | T35～T46 | 已完成 | 一致 | T45 已在 macOS/Windows 通过 15 条真实桌面链与成对 artifact；T46 阶段验收通过 |
| 必须实现 | R14 | 一目录一窗口与多窗口生命周期 | 页签会话、偏好、全页签替换保护和独立窗口恢复 | T36、T40～T46 | 已完成 | 部分覆盖 | T42 已恢复独立窗口页签；T45 双平台验证单窗口重启与当前窗口替换拒绝，位置/尺寸/三栏布局属阶段 4，真实多窗口整组退出/恢复仍未完成 |
| 必须实现 | R15 | 颜色主题预设与实时预览 | 暂不纳入 | - | 跳过 | 未覆盖 | 阶段 5 实现，P3 不注册 |
| 必须实现 | R30 | 核心命令键盘操作 | 页签切换/关闭/重开、关闭窗口与弹层焦点子集 | T38～T46 | 已完成 | 部分覆盖 | T43 已完成页签菜单和聚焦窗口路由；T45 已用 macOS/Windows 真实系统输入验证平台页签组合键，搜索、阅读和布局命令留对应任务/阶段 |
| 必须实现 | R31 | 无障碍基础 | tablist/menu/dialog 的语义、焦点、非颜色状态与减少动态效果 | T38、T40、T43～T46 | 已完成 | 部分覆盖 | T43 已完成无目标禁用和原生“所有页签”到共享菜单的焦点入口；完整桌面无障碍矩阵仍属阶段 4 |
...
- 实际落地情况：已新增 `WorkspaceTabManager` 与薄 `tabSessionGateway`，P1 文件树打开 Markdown 时先消费 T36 Rust 返回的规范化相对路径和 opaque 平台身份，再创建或聚焦唯一 runtime。每个已加载页签独立持有 `DocumentSession`、统一 history、选择/锚点、模式和 `DocumentSaveController`；非活动 dirty 页签继续自动保存与恢复快照，慢读取通过 tabId/incarnation/generation 三重校验隔离，慢结果也不能回写活动窗口标题。
  ...
  - P1 的中央内容、文件树选择、状态栏、标题和原生菜单继续只消费活动 runtime。冲突重载与恢复副本重新读取目标磁盘基线；恢复命中已有 dirty runtime 时先执行内容保护。窗口关闭/替换/退出 intent 的可见批量决策由 T40 建立，窗口 coordinator 接线仍归 T41。
...
- 实际落地情况：已新增 `WorkspaceTabBar`、共享 `TabMenu`、`TabOverflowMenu` 与 `TabContextMenu`，由 P1 直接消费 `WorkspaceTabManager` 快照。页签条显示活动项、同名父路径提示和由 T35 公共优先级投影的 unloaded/loading/dirty/saving/readonly/conflict/missing/error 等状态；图形、文字与可访问名称共同表达，不依赖颜色。
  - 已实现鼠标激活/关闭、当前窗口拖动排序、方向键/Home/End 切换、`Alt+Shift+←/→` 键盘排序、Delete 关闭及 Shift+F10/ContextMenu 菜单。tablist 使用 roving tabindex；溢出和上下文菜单共同消费 `TabMenu` 与既有 `focusableElements`，支持方向键、首尾、Esc、外部点击和焦点返回。
  - 单页签关闭复用现有 manager/controller 结算：关闭前提交活动 adapter 投影，安全关闭后再切换树选择、窗口标题和活动 runtime；阻塞时保持页签并给出真实原因。T40 已让关闭其他/右侧/全部消费同一两阶段结算；最近关闭入口与顺序持久化由 T39 完成。
  - 工作台网格增加稳定 38 px 页签行；页签自身拥有横向滚动，820/740 px 不产生根级横向溢出，菜单不建立私有颜色或层级体系。组件测试覆盖同名路径、50 项溢出、键盘、焦点、拖动、上下文菜单和安全关闭；真实 macOS Tauri/WebKit 10/10 覆盖三文档真实页签、点击切换、溢出菜单焦点返回、1100/820/740 px 与单 adapter/RSS 门禁。
  - 本地验证：Vitest 26 文件 219/219、Node 独立回归 30/30、Rust 200 通过且 1 项手动探针忽略、类型检查、生产构建、许可证 727/511/0 和 macOS 桌面 E2E 10/10。未修改 Rust、依赖、菜单、权限或 capability，未重跑 Windows/远端 CI；Windows 与双平台证据不得从第二阶段外推。
...
- 实际落地情况：已扩展 `WorkspaceTabManager` 与页签 reducer，最近关闭项通过统一 `open()` 重新请求 Rust 路径身份后才恢复；失败项留在列表并显示稳定错误，可显式移除最近记录且不删除磁盘文件。`WorkspaceTabBar` 的溢出菜单复用 `TabMenu`，在全部打开页签之后展示最近关闭区，成功重开后恢复可兼容的模式、选择和锚点。
  - 新增 `tabSessionProjection.ts` 与 `WorkspaceTabSessionPersistence`：把有序页签、活动项、轻量视图和最近关闭投影为不含 Markdown/history 的会话 DTO，以 180 ms 防抖和 Rust revision/CAS 保存，并只把实际已提交的 collection revision 标记为已持久化。若仓储已经存在非空会话，T39 不覆盖尚未由 T42 恢复的元数据；该情形明确推迟到 T42 完成读取、惰性激活与后续写入。
  - 新增纯 `tabPathImpact` 分析器，rename/move/delete 在磁盘调用前收集全部受影响页签、目标相对路径及所有已加载文档的内联图片链接改写预案。该任务结束时对命中打开页签的操作采取临时安全阻断；T40 已用两阶段结算与磁盘成功后的批量 runtime 重映射替代该过渡行为。
  - 组件、manager、路径影响、投影、持久化和 P1 集成测试覆盖重开失败保留、成功恢复、失效记录移除、顺序 CAS、既有会话不覆盖、revision 冲突停写、多页签目录移动/删除影响和图片链接预案。真实 Tauri E2E 增加干净页签关闭、最近列表重开以及 Rust 会话顺序/最近项断言；完整结果和未验证项见 `t39-tab-file-integration.md`。
...
- 实际落地情况：已先新增并复核 `prototypes/tab-settlement-dialog.html`：1100×760 和 560×720 两档均无根级横向溢出，固定标题/动作区与独立滚动清单成立；dirty/conflict/readonly 等混合项未解决时最终动作禁用，放弃修改须二次确认，全部解决后才允许提交。
  - 新增纯 `tabSettlement` 批次模型与 `TabSettlementDialog`。目标以 `tabId + incarnation` 固定，显式放弃/另存证据绑定 `generation + editVersion`；继续编辑会使旧证据失效。逐项复用现有 `DocumentSaveController`、`ConflictDialog`、`SaveCopyDialog` 和 `AppDialog`，没有第二套保存、冲突或文件选择通道。
  - `WorkspaceTabManager` 新增全目标结算、单 revision 批量关闭和路径/runtime 原子重映射。所有目标在移除前先完成 incarnation 复核；评审整改后，最终关闭目标还固定结算时的 session generation/editVersion，并在异步释放恢复身份前后复核。任一目标阻塞、陈旧或在磁盘删除期间继续编辑时不移除任何页签；批次取消只保留页签，不回滚已经真实成功的磁盘保存。
  - 页签上下文菜单已启用关闭其他、关闭右侧、关闭全部。rename/move/delete 命中打开页签时不再使用 T39 临时阻断：先完成结算，Rust 磁盘操作成功后才批量提交路径身份、树状态或页签移除；磁盘失败保持原页签集合和原路径。目录移动后的内联图片链接对全部受影响已加载页签重新计算并进入各自保存链；写回失败时目录移动事实不被伪装回滚，相关页签保持 dirty 并展示明确提示。
  - 真实桌面移动回归发现并修复了活动 adapter 在 React 重投影前覆盖 manager 新图片链接的竞态：manager 生成的重映射保存跳过一次陈旧 adapter 采集，普通编辑/关闭仍先提交活动投影。前端专项 63/63、全量 Vitest 249/249、Node 独立回归 30/30、Rust 200 项通过且 1 项手动探针忽略、类型检查、生产构建、Rust fmt/全 feature Clippy、许可证 727/511/0 与 macOS Tauri/WebKit 桌面 E2E 11/11 均通过。T40 未修改 Rust、capability、依赖、原生菜单或产品配置；Windows 和远端 CI 未覆盖第三阶段提交，不能外推为双平台桌面验收。完整证据见 `t40-tab-settlement.md`。

### 6.7 任务 T41：窗口替换、关闭/退出协调与打开偏好

- 状态：已完成。
- 依赖：T36、T40。
- 涉及文件/模块：`WorkspaceWorkbench`、`WorkspaceLauncher`、`WorkspaceOpenDecisionDialog`、`WorkspaceOpenPreferenceDialog`、`preferences.rs`、`window.rs`、`menu.rs`、gateway/contracts、测试。
- 目标：完成 R14 的全页签替换保护和可回退打开偏好，并保持现有窗口事务纪律。
- 操作：扩展偏好字段/命令；激活 P2 设置入口和原生设置菜单；把 replace/close/quit settlement 从单 controller 改为 tab collection；正文全部安全后尽力持久化旧会话，再允许 Rust 提交；保持新窗口/同目录聚焦路径。
...
- 实际落地情况：已在 `plainroot-preferences-v1.json` 的 schema v1 中增加向后兼容的可选全局 `workspaceOpenDisposition`，Rust/TypeScript 共同约束 `ask/current_window/new_window` 三值；缺失字段默认 `ask`，保存采用既有原子仓储，写失败保持磁盘与内存旧值，并提供读取、保存、重置命令。
  - P1/P2 共同消费 `WorkspaceOpenDecisionDialog` 与 `WorkspaceOpenPreferenceDialog`。P2 设置按钮和 macOS 应用菜单/Windows 文件菜单的“工作区打开方式…”进入同一偏好弹层；打开调用默认读取偏好，读取失败安全回退为询问，同目录仍由既有 Rust coordinator 聚焦原窗口，偏好不能跳过授权或窗口结算。
  - P1 已将 Rust 的 `replace_workspace/close_window/quit_app` intent 映射到 T40 的同一不可变全页签结算批次。任一 dirty/save_failed/readonly/conflict 未解决时阻止最终动作；取消会显式向 Rust 拒绝 intent，全部安全后尽力持久化当前内容无关页签会话，再允许 Rust 提交替换、关闭或整组退出。评审整改后，会话仓储仍在初始化或已写失败时不再困住已安全窗口，而是显示非阻断提示并继续提交；旧的有效元数据仍可在下次启动恢复，最坏只退回较早页签状态。最终窗口提交失败仍保留页签和批次，可重试或取消，不虚报窗口事务已回滚。
  - T42 尚未消费的既有非空页签会话继续保持 `deferred_existing_session` 冻结，不由 T41 覆盖；窗口位置、尺寸、布局和启动恢复未进入本任务。没有新增 capability、数据库、SQL、seed、产品环境变量或依赖。
  - 本地验证通过：255/255 Vitest、201/201 Rust（另 1 项手动性能探针忽略）、30/30 Node 独立回归、63/63 页签专项、类型检查、生产构建、Rust fmt/全 feature Clippy、许可证 727/511/0 和 macOS 真桌面 E2E 11/11；新增组件回归证明页签会话元数据写失败时，已安全文档仍允许窗口关闭并给出明确降级提示。浏览器对 P2 设置入口执行了 1100×760 与 740×760 两档实测，无根级横向溢出，弹层可见且取消后焦点返回触发器。Windows、远端 CI、真实双窗口退出/替换桌面 E2E 和原生设置菜单人工验收未在本任务执行，留给 T45～T46。完整证据见 `t41-window-lifecycle.md`。
...
- 实际落地情况：
  - `src-tauri/src/menu.rs` 新增单一原生“页签”菜单，提供关闭当前、重开最近关闭、下一/上一、关闭其他/右侧/全部和“所有页签…”；关闭窗口继续使用 `Cmd/Ctrl+Shift+W`。所有页签动作继续发到既有 `WORKBENCH_MENU_EVENT`，未创建第二套事件总线或 React 全局 accelerator。
  - `EditorMenuState` 增加内容无关的 `TabMenuState(tabCount/activeTabIndex/recentlyClosedCount)`；Rust 只对当前聚焦窗口应用启用态，busy、无活动页签、无右侧项或无最近关闭时精确禁用。P2/无页签 P1 自动投影零值，不发送空成功。
  - P1 的现有 `workbenchMenuHandlerRef` 把原生命令委托 `WorkspaceTabManager`、`closeWorkspaceTabs` 和共享 `TabMenu`；关闭当前/其他/右侧/全部继续消费 T40 的一次性全页签结算，重开继续重新解析 Rust 路径身份，“所有页签…”只请求既有溢出菜单打开并把焦点交给首个可用菜单项。
  - Rust/TypeScript action、`EditorMenuState` 与字符串常量继续受 parity 守卫；新增 Rust 菜单策略和双窗口 registry 隔离测试，以及页签栏/工作台组件路由与焦点测试。没有新增视觉组件、CSS、数据库、SQL、seed、capability、持久化字段、产品环境变量、依赖或初始化数据。
  - 本地验证通过：70/70 页签专项、267/267 Vitest、203/203 Rust（另 1 项手动性能探针忽略）、30/30 Node 独立回归、类型检查、生产构建、Rust fmt、全 feature Clippy/测试、许可证 727/511/0、当前代码 macOS Tauri/WebKit 11/11。首次尝试用 WebDriver 注入原生快捷键只到达 WebView，相关无效断言已移除且未写入测试；随后以隔离 E2E `.app` 做原生走查，确认菜单与启用态可见、点击“下一个页签”切换活动页签、`Cmd+W` 关闭当前页签、`Cmd+Shift+T` 重开最近关闭。`Ctrl+Tab` 的 Computer Use 注入未触发，Windows 与远端 CI 未执行，继续由 T45～T46 承接。完整证据见 `t43-tab-menu-keyboard.md`。
...
- 完成标准：具备清单声明工具链的新 checkout 可用一条统一命令复现所有非桌面状态与服务门禁；当前实际 204 Rust/267 Vitest/30 Node 基线不回退，精确计数仍以测试发现结果而非手写常量为准。
- 实际落地情况：
  - `src-tauri/src/contract_test.rs` 将 tagged-union parity 从“全部变体字段并集”收紧为按 `kind` 标签逐变体比较字段集合；TypeScript 常量索引会解析为真实标签，Rust fixture 也必须提供唯一字符串标签。字段被放错变体、重复/缺失标签、索引越界或结构不完整都会失败。
  - 新增反向测试故意交换 `WindowTabSelection` 的 visual/source 字段，确认旧守卫的盲区已被关闭；正确的 `WindowTabSelection`/`WindowTabAnchor` Rust↔TypeScript 契约继续通过。
  - `package.json` 新增 `test:stage-3:contracts`，统一执行页签/工作台专项和完整无桌面 Rust 服务门禁；新增 `verify:non-desktop`，一条命令串联 Node、Vitest、no-default/all-features Rust、fmt、Clippy、typecheck、生产构建和许可证检查。现有临时 workspace/app-data 故障注入矩阵保持隔离，未创建第二套 fixture 或测试框架。
  - `pnpm test:stage-3:contracts` 通过 110/110 Vitest 与 204 项 Rust（另 1 项手动性能探针忽略）；`pnpm verify:non-desktop` 从头通过 30/30 Node、267/267 Vitest、204 项 no-default Rust与 204 项 all-features Rust（均另 1 项忽略）、fmt、全 target Clippy、typecheck、生产构建和许可证 727/511/0。首次总门禁在 fmt 差异处真实失败，格式化后重跑才通过，证明入口会阻止失败。
  - T44 未修改生产页面、IPC、磁盘格式、数据库、SQL、seed、capability、菜单、产品配置、环境变量、依赖或初始化数据；没有新的页面可视验证。完整证据见 `t44-tab-contract-tests.md`。
...
- 实际落地情况：
  ...
  - 第十五次远端 run `30322298422` 对提交 `9ba28d1bd077b950059b6b474e7da77cf173ea63` 已让 macOS/Windows 两个作业正式 `success`：统一非桌面门禁、四段 15/15 真实桌面链、未签名生产构建和 artifact 上传全部完成。Windows 首次以真实系统 `Ctrl+PageDown/PageUp` 通过页签双向切换，并继续通过主链、重启种子和恢复隔离；macOS 保持原生菜单 `Cmd+Option+Right/Left`。`plainroot-macos-30322298422` SHA-256 为 `15541ac7491602e740d5d93518ab1d58c258ee3c0da442c7ee0d82a346a9bb95`，`plainroot-windows-30322298422` SHA-256 为 `4da211e9404884e3119b0854034ebf094bd42f4218adf62b10bcd7a500e5e23e`。T45 完成；后续 T46 阶段验收见本节 6.12。
  - 第十六次远端 run `30323620642` 对纯文档提交 `a0268eb2482face230bb6458f085a68afcfaeaed` 再次让 macOS/Windows 两个作业正式 `success`，证明首轮收口文档没有脱离代码完成证据。`plainroot-macos-30323620642` SHA-256 为 `d16334981a0f6a6c7be01734b80a67da1acaf21f72a237d4be5a44d28aeaba07`，`plainroot-windows-30323620642` SHA-256 为 `482ca21af722bf0b66980c59844a87408921a2a798d2d2de9bf923d468315ced`。完成判据以第十五次代码 run 为主、以第十六次收口文档 run 为补充，不混用提交归属。
...
- 实际落地情况：
  - 已完成代码正确性、业务闭环、页面 UX、前端复用、DESIGN、权限、配置、菜单和文档一致性复核；未发现新的阻断缺陷。
  - 新增 `architecture/tab-window-lifecycle.md` 和 `t46-stage-acceptance.md`，逐项核对 R1～R34、T35～T46，并明确 R13 已完成、R14/R30/R31 仍为产品范围的部分覆盖。
  - 当前本地 `pnpm verify:non-desktop` 通过：Node 30/30、Vitest 269/269、Rust no-default/all-features 各 207 项通过且 1 项手动探针忽略，fmt、Clippy、类型、生产构建和许可证 727/511/0 通过；`pnpm tauri build --no-bundle` 通过。
  - 当前本地桌面 E2E 两次均先通过主链 12/12，随后因 macOS `System Events` 未找到 fixture 标题窗口而停在原生快捷键前台定位，未进入业务断言，后续两段未执行；本次不记为 15/15。T45 代码 run `30322298422` 与纯文档 run `30323620642` 的双平台 15/15、生产构建和成对 artifact 仍是阶段桌面完成证据。
  - SQL/seed/产品环境变量无新增；capability、菜单和脚本没有 T46 代码变更。稳定规则只同步最新边界，不写入阶段流水账。
...
- 验证 `WorkspaceTabBar`、共享页签菜单基元、`TabSettlementDialog`、`WorkspaceOpenDecisionDialog`、`WorkspaceOpenPreferenceDialog` 与 `WorkspaceTabManager` 均被 4.9 表中的真实页面消费，不存在只登记未接线的资产。
...
- 保持既有 P1/P2 桌面 E2E，并增加页签、窗口 intent 与跨进程恢复用例；不得通过删除断言或全局重试换绿。
...
| 目录操作影响多个打开文档 | 路径/recovery/图片链接漂移 | T40 已在操作前结算集合、磁盘成功后批量重映射，并以 macOS 真实磁盘 E2E 验证打开页签改名/移动/删除及图片链接写回；写回失败保持 dirty/真实提示，T45 继续补双平台与故障回归 |
| 平台快捷键重复分发 | 重复切换/关闭或弹层 | macOS 页签切换 accelerator 只在 Rust；Windows 只在聚焦 WebView 注册 `Ctrl+PageDown/PageUp`，Rust 菜单不重复绑定同组合键；其余组件只处理局部 tablist 键盘 |
...
- P1 原型未覆盖的安全关键多页签混合结算已由 T40 按 4.5.1 补充原型并通过两档页面复核，没有暴露需要新增确认的关闭结果选择；该待补项已闭合。
