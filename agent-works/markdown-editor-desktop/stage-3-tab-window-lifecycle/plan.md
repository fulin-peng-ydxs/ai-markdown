...
>
> 计划状态：执行中（T35～T45 已完成；代码 run `30322298422` 已覆盖提交 `9ba28d1` 的全部 T45 实现，收口文档 run `30323620642` 已覆盖其上的纯文档提交 `a0268eb`，两次均为 macOS/Windows 双绿与成对 artifact；T46 未实施）
>
...
| --- | --- | --- | --- | --- | --- |
| R1 | 在现有 Windows/macOS 壳内增加真实多页签和会话恢复，不重做安装/授权 | 进行中 | 部分覆盖 | T41～T46 | T45 run `30322298422` 已在 macOS/Windows 验证真实多页签、重启恢复与单项隔离；T46 阶段验收仍待完成 |
| R2 | 文件树打开、重命名、移动、删除同步作用于全部相关页签 | 进行中 | 部分覆盖 | T37、T39～T40、T44～T45 | T40 已在磁盘成功后一次提交全部 runtime 路径/身份和树状态；T45 双平台主链已回归真实改名、移动、删除及图片链接写回 |
| R3 | 每页签独立消费现有排版编辑与统一 history | 进行中 | 部分覆盖 | T35、T37、T39、T44～T45 | T37 已完成每页签独立 session/history 与活动 editor 单挂载，T45 双平台桌面链已回归多页签编辑与单 adapter |
| R4 | 阶段 6 实现大纲；本阶段只保证活动页签单一来源可供未来消费 | 跳过 | 未覆盖 | - | 确认无假大纲入口 |
| R5 | 将保存、恢复、冲突和关闭保护扩展为全页签结算 | 进行中 | 部分覆盖 | T37、T40～T46 | T40～T42 已闭合全页签结算、窗口事务和恢复；T45 双平台验证真实替换拒绝、跨进程恢复和既有保存/冲突主链，T46 阶段验收仍待完成 |
| R6 | 保持每页签图片链路；目录/文件移动时处理全部相关打开文档的链接和状态 | 进行中 | 部分覆盖 | T37、T39～T40、T44～T45 | T40 已在路径提交后对全部受影响已加载页签重算内联图片链接并进入各自保存链；T45 双平台主链已回归真实磁盘移动与图片链接写回 |
...
| R10 | 每页签独立保留排版/源码模式和源码选择 | 进行中 | 部分覆盖 | T35、T37、T42、T44～T45 | T42 已恢复各页签模式、选择和锚点元数据并惰性加载真实内容；T45 双平台跨进程恢复通过 |
| R11 | 页签加载、保存、冲突、缺失、恢复和批量结算状态真实可见 | 进行中 | 部分覆盖 | T38～T46 | T43/T44 已固化聚焦窗口菜单与非桌面状态契约；T45 双平台验证桌面矩阵与恢复缺失项反馈，完整验收仍属阶段 4 |
| R12 | 阶段 7 实现工作区全文搜索 | 跳过 | 未覆盖 | - | 搜索菜单继续禁用 |
| R13 | 完整纳入：真实页签、唯一打开、排序、批量关闭、溢出、最近关闭、独立会话和恢复 | 进行中 | 一致 | T35～T46 | T45 已在 macOS/Windows 通过 15 条真实桌面链，含平台页签组合键、替换拒绝、跨进程恢复与成对 artifact；T46 阶段验收仍待完成 |
| R14 | 纳入页签会话、打开偏好、全页签替换保护和独立窗口恢复；位置/尺寸/三栏布局留阶段 4 | 进行中 | 部分覆盖 | T36、T40～T46 | T42 已消费根会话引用并恢复独立窗口页签集合，T45 双平台验证单窗口跨进程恢复、缺失项隔离和当前窗口替换拒绝；真实多窗口整组退出/恢复与阶段验收仍待完成 |
| R15 | 阶段 5 实现主题工作室 | 跳过 | 未覆盖 | - | P3 不注册 |
| R30 | 纳入页签切换/关闭、重新打开、关闭窗口和弹层焦点子集 | 进行中 | 部分覆盖 | T38～T46 | T45 已用 macOS `Cmd+Option+Right/Left` 与 Windows `Ctrl+PageDown/PageUp` 真实系统输入验证页签双向切换；T46 子集验收仍待完成 |
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| 必须实现 | R1 | Windows/macOS 本地优先桌面应用 | 在既有桌面壳内接入多页签、窗口会话和双平台回归 | T41～T46 | 进行中 | 部分覆盖 | T45 run `30322298422` 已在 macOS/Windows 验证真实多页签、重启恢复与单项隔离；T46 阶段验收仍待完成 |
| 必须实现 | R2 | 文件与目录管理 | 文件树入口、CRUD 与全部相关页签联动 | T37、T39～T40、T44～T45 | 进行中 | 部分覆盖 | T40 已在磁盘成功后提交路径/runtime 或页签移除；T45 双平台主链已回归真实改名、移动、删除与图片链接写回 |
| 必须实现 | R3 | 所见即所得 Markdown 编辑 | 每页签独立消费既有 DocumentSession/editor | T35、T37、T39、T44～T45 | 进行中 | 部分覆盖 | 独立 session/history 与活动 editor 单挂载已纳入统一门禁；T45 双平台回归真实多页签编辑与单 adapter |
| 必须实现 | R4 | 当前文档大纲 | 暂不纳入 | - | 跳过 | 未覆盖 | 阶段 6 实现；活动页签只预留单一来源 |
| 必须实现 | R5 | 自动保存、恢复和冲突 | 把单文档保护扩展为关闭、替换、退出的全页签结算 | T37、T40～T46 | 进行中 | 部分覆盖 | 恢复与全页签结算已通过非桌面故障矩阵；T45 双平台验证真实替换拒绝、跨进程恢复和既有保存/冲突主链，T46 阶段验收待完成 |
| 必须实现 | R6 | 图片粘贴、拖放与资源管理 | 多页签路径/图片链接随文件树操作一致变更 | T37、T39～T40、T44～T45 | 进行中 | 部分覆盖 | 全部受影响已加载页签的链接重算与失败保持 dirty 已纳入门禁；T45 双平台回归真实磁盘移动和图片链接写回 |
...
| 必须实现 | R10 | Markdown 源码模式 | 每页签独立保留 visual/source 模式、选择与锚点 | T35、T37、T42、T44～T45 | 进行中 | 部分覆盖 | T42 已恢复模式、选择与锚点元数据并惰性加载正文；T45 双平台跨进程恢复通过 |
| 必须实现 | R11 | 操作与文件状态反馈 | 实现页签、结算、恢复和持久化新增状态 | T38～T46 | 进行中 | 部分覆盖 | 页签/结算/恢复/菜单状态已纳入非桌面门禁；T45 双平台验证桌面矩阵和恢复缺失项反馈，完整验收仍属阶段 4 |
| 必须实现 | R12 | 工作区全文搜索 | 暂不纳入 | - | 跳过 | 未覆盖 | 阶段 7 实现；页签列表不冒充搜索 |
| 必须实现 | R13 | 多文档页签与最近关闭文档 | 完整实现真实页签、排序、批量关闭、最近关闭和会话恢复 | T35～T46 | 进行中 | 一致 | T45 已在 macOS/Windows 通过 15 条真实桌面链与成对 artifact；T46 阶段验收仍待完成 |
| 必须实现 | R14 | 一目录一窗口与多窗口生命周期 | 页签会话、偏好、全页签替换保护和独立窗口恢复 | T36、T40～T46 | 进行中 | 部分覆盖 | T42 已恢复独立窗口页签；T45 双平台验证单窗口重启与当前窗口替换拒绝，位置/尺寸/三栏布局属阶段 4，真实多窗口整组退出/恢复及 T46 验收仍待完成 |
| 必须实现 | R15 | 颜色主题预设与实时预览 | 暂不纳入 | - | 跳过 | 未覆盖 | 阶段 5 实现，P3 不注册 |
| 必须实现 | R30 | 核心命令键盘操作 | 页签切换/关闭/重开、关闭窗口与弹层焦点子集 | T38～T46 | 进行中 | 部分覆盖 | T43 已完成页签菜单和聚焦窗口路由；T45 已用 macOS/Windows 真实系统输入验证平台页签组合键，搜索、阅读和布局命令留对应任务/阶段 |
