...
>
> 计划状态：执行中（T35～T41 已完成；T42～T46 未实施）
>
...
- 2026-07-27 已在 T41 当前本地状态重新执行门禁：Node 独立回归 30/30、Vitest 32 个文件 255/255、Rust 201 项通过且 1 项手动性能探针忽略、类型检查、生产构建、Rust fmt/全 feature Clippy 和许可证 727/511/0 通过；页签专项仍为 63/63，macOS Tauri/WebKit 桌面套件 11/11 通过。T41 测试覆盖三值打开偏好、向后兼容、保存/重置/失败回滚、P1/P2 双入口、mixed 页签集合对替换/退出 intent 的取消与最终提交，以及内容已安全时页签会话元数据写失败仍允许优雅关闭；既有真桌面套件证明 P1/P2、批量结算和磁盘链路未回退，但没有新增 T41 窗口事务 E2E，完整双平台验收仍属 T45。各任务留痕中的测试数字只是当时的历史快照。最新远端证据仍为 GitHub Actions run `30082725332` 在 macOS/Windows 通过第二阶段 9 条套件，尚未覆盖第三阶段本地提交。
...
- `WorkspaceWorkbench` 已消费唯一 `WorkspaceTabManager`，页面上的 `documentState`、`editorHandleRef` 和 `saveControllerRef` 只是活动 runtime 投影；T38 已接入可见页签层，T39 已接入最近关闭重开、当前会话元数据写入和路径影响预分析，T40 已接入全页签两阶段结算、批量关闭和磁盘成功后的多 runtime 原子重映射；已有非空会话的启动恢复仍由 T42 完成。
...
- 新增页签上下文菜单、最近关闭列表、全页签结算弹层和打开方式设置弹层没有完整上游 HTML 原型；其流程由 R5/R13/R14/R30、7.1.6、7.1.7、7.2.3 明确定义。普通菜单与少字段偏好弹层直接复用现有区域、`AppDialog` 和 DESIGN 规范；T40 已先新增 `prototypes/tab-settlement-dialog.html`，按 1100×760 与 560×720 两档复核混合状态、滚动、明确动词、取消和最终提交门禁后，才实现生产组件。打开方式设置弹层仍由 T41 承接。
...
| --- | --- | --- | --- | --- | --- |
| R1 | 在现有 Windows/macOS 壳内增加真实多页签和会话恢复，不重做安装/授权 | 进行中 | 部分覆盖 | T41～T46 | T41 已在既有壳接入打开偏好、设置菜单和窗口结算；启动会话恢复与双平台桌面验收仍待 T42/T45～T46 |
...
| R5 | 将保存、恢复、冲突和关闭保护扩展为全页签结算 | 进行中 | 部分覆盖 | T37、T40～T46 | T40 已完成不可变批次和一次性提交；T41 已让窗口替换、关闭和退出消费同一批次，启动恢复与阶段回归仍待 T42、T44～T46 |
...
| R11 | 页签加载、保存、冲突、缺失、恢复和批量结算状态真实可见 | 进行中 | 部分覆盖 | T38～T46 | T40 已以非颜色逐项状态、内容安全说明和明确下一步呈现批量结算；启动恢复与完整阶段状态矩阵仍待 T42～T46 |
| R12 | 阶段 7 实现工作区全文搜索 | 跳过 | 未覆盖 | - | 搜索菜单继续禁用 |
| R13 | 完整纳入：真实页签、唯一打开、排序、批量关闭、溢出、最近关闭、独立会话和恢复 | 进行中 | 一致 | T35～T46 | T35～T40 已完成模型、仓储、路径身份、独立 runtime、可见页签、排序/溢出/最近关闭，以及当前/其他/右侧/全部的安全批量关闭；已有会话启动恢复仍待 T42，原生命令与阶段验收待后续 |
| R14 | 纳入页签会话、打开偏好、全页签替换保护和独立窗口恢复；位置/尺寸/三栏布局留阶段 4 | 进行中 | 部分覆盖 | T36、T40～T46 | T41 已完成打开偏好和全部页签替换/关闭/退出保护；已有会话启动恢复、失败隔离和双平台验收仍待 T42、T44～T46 |
| R15 | 阶段 5 实现主题工作室 | 跳过 | 未覆盖 | - | P3 不注册 |
| R30 | 纳入页签切换/关闭、重新打开、关闭窗口和弹层焦点子集 | 进行中 | 部分覆盖 | T38～T46 | T41 已接通窗口级结算和共享打开偏好弹层焦点；原生页签快捷键、命令路由与阶段验收仍待 T43～T46 |
| R31 | 纳入 tablist/menu/dialog 的语义、焦点、非颜色状态和减少动态效果子集 | 进行中 | 部分覆盖 | T38、T40、T43～T46 | T40 已落地安全结算 dialog 的逐项非颜色状态、焦点和明确动词；完整对比度、系统偏好与阶段 4 矩阵仍待后续 |
...
| P1/P2 页签会话能力 | 新增 `WorkspaceTabManager`，复用 `DocumentSession`、`DocumentSaveController`、recovery/assets gateway | 新增非视觉运行时状态能力 | 不注册页面；通过 manager provider/props 供 P1，P2 恢复后创建 | DESIGN 运行时复用清单登记稳定职责；架构文档记录数据边界 | P1 活动会话与最近关闭真实消费，P2 恢复待 T42；manager、集成和 E2E | T35、T37、T39、T42、T44～T46 |
| P1 页签会话持久化 | 新增 `WorkspaceTabSessionPersistence`，复用 T36 Rust window-session gateway | 非视觉防抖/CAS 调度能力 | 输入 manager 快照、workspaceId 与错误回调；不注册页面或配置 | DESIGN 登记只保存轻量元数据且不得覆盖未恢复既有会话 | P1 顺序/活动/最近项真实写入；投影、冲突和桌面 IPC 测试 | T39、T42、T44～T45 |
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| 必须实现 | R1 | Windows/macOS 本地优先桌面应用 | 在既有桌面壳内接入多页签、窗口会话和双平台回归 | T41～T46 | 进行中 | 部分覆盖 | T41 已接入设置菜单和窗口结算；启动恢复/双平台回归待后续任务 |
...
| 必须实现 | R5 | 自动保存、恢复和冲突 | 把单文档保护扩展为关闭、替换、退出的全页签结算 | T37、T40～T46 | 进行中 | 部分覆盖 | T41 已完成窗口替换/关闭/退出接线；启动恢复和阶段验收待 T42、T44～T46 |
...
| 必须实现 | R11 | 操作与文件状态反馈 | 实现页签、结算、恢复和持久化新增状态 | T38～T46 | 进行中 | 部分覆盖 | T38 已承接可见页签状态；结算/恢复仍待后续，完整验收仍属阶段 4 |
| 必须实现 | R12 | 工作区全文搜索 | 暂不纳入 | - | 跳过 | 未覆盖 | 阶段 7 实现；页签列表不冒充搜索 |
| 必须实现 | R13 | 多文档页签与最近关闭文档 | 完整实现真实页签、排序、批量关闭、最近关闭和会话恢复 | T35～T46 | 进行中 | 一致 | T35～T40 已完成模型、仓储、路径身份、P1 runtime、可见交互、最近关闭、元数据写入和安全批量关闭；已有会话启动恢复与阶段验收仍待后续任务 |
...
| 必须实现 | R30 | 核心命令键盘操作 | 页签切换/关闭/重开、关闭窗口与弹层焦点子集 | T38～T46 | 进行中 | 部分覆盖 | T38 已完成页签切换/关闭/排序和菜单焦点子集，T39 已完成菜单重开最近关闭；原生重开快捷键、窗口、搜索、阅读和布局命令留对应任务/阶段 |
| 必须实现 | R31 | 无障碍基础 | tablist/menu/dialog 的语义、焦点、非颜色状态与减少动态效果 | T38、T40、T43～T46 | 进行中 | 部分覆盖 | T38 已完成 tablist/menu 子集；完整桌面无障碍矩阵仍属阶段 4 |
