...
>
> 计划状态：执行中（T35～T39 已完成；T40～T46 未实施）
>
...
- 2026-07-26 已在 T39 当前本地状态重新执行门禁：Node 独立回归 30/30、Vitest 29 个文件 232/232、Rust 200 项通过且 1 项手动性能探针忽略、类型检查、生产构建、许可证 727/511/0 和 macOS Tauri/WebKit 桌面套件 10/10 通过。T37 已取得 Rust fmt/全 feature Clippy 与锁定 e2e check；T39 未修改 Rust、依赖或 capability，因此未重复执行这些未受影响的门禁。各任务留痕中的测试数字只是当时的历史快照。最新远端证据仍为 GitHub Actions run `30082725332` 在 macOS/Windows 通过第二阶段 9 条套件，尚未覆盖第三阶段本地提交。
...
- `WorkspaceWorkbench` 已消费唯一 `WorkspaceTabManager`，页面上的 `documentState`、`editorHandleRef` 和 `saveControllerRef` 只是活动 runtime 投影；T38 已接入可见页签层，T39 已接入最近关闭重开、当前会话元数据写入和路径影响预分析；已有非空会话的启动恢复与多项阻塞决策仍由 T42、T40 完成。
...
| R2 | 文件树打开、重命名、移动、删除同步作用于全部相关页签 | 进行中 | 部分覆盖 | T37、T39～T40、T44～T45 | T37 已让文件树打开/聚焦真实 runtime；T39 已在磁盘操作前收集全部受影响页签并阻止尚不能原子结算的改名/移动/删除，实际磁盘提交与全体 runtime 重映射仍由 T40 承接 |
...
| R6 | 保持每页签图片链路；目录/文件移动时处理全部相关打开文档的链接和状态 | 进行中 | 部分覆盖 | T37、T39～T40、T44～T45 | T37 已保持活动页签既有图片链路并隔离 session；T39 已生成全部已加载文档的图片链接改写预案且不修改磁盘，实际安全结算与提交仍待 T40 |
...
| R11 | 页签加载、保存、冲突、缺失、恢复和批量结算状态真实可见 | 进行中 | 部分覆盖 | T38～T46 | T38 已按公共优先级显示当前页签状态并提供非颜色文字；恢复和批量结算状态仍待后续任务 |
| R12 | 阶段 7 实现工作区全文搜索 | 跳过 | 未覆盖 | - | 搜索菜单继续禁用 |
| R13 | 完整纳入：真实页签、唯一打开、排序、批量关闭、溢出、最近关闭、独立会话和恢复 | 进行中 | 一致 | T35～T46 | T35～T39 已完成状态机、仓储、Rust 路径身份、真实多 session runtime、可见页签条、单项关闭、排序、溢出、最近关闭重开及当前会话元数据持久化；已有会话启动恢复、批量动作与完整结算未实施 |
...
| R30 | 纳入页签切换/关闭、重新打开、关闭窗口和弹层焦点子集 | 进行中 | 部分覆盖 | T38～T46 | T38 已提供页签方向键/首尾切换、Delete 关闭、键盘排序和菜单焦点返回；T39 已提供溢出菜单中的最近关闭重开与失效记录安全移除，原生快捷键和窗口批量流程仍待后续任务 |
| R31 | 纳入 tablist/menu/dialog 的语义、焦点、非颜色状态和减少动态效果子集 | 进行中 | 部分覆盖 | T38、T40、T43～T46 | T38 已落地 tablist/menu 语义、roving tabindex、非颜色状态和 reduced-motion；完整对比度与系统偏好回归仍属后续和阶段 4 |
...
| 必须实现 | R2 | 文件与目录管理 | 文件树入口、CRUD 与全部相关页签联动 | T37、T39～T40、T44～T45 | 进行中 | 部分覆盖 | T37 已让文件树打开/聚焦真实 runtime；T39 已完成磁盘提交前影响收集和安全阻断，实际路径重映射与删除联动仍待 T40 |
...
| 必须实现 | R6 | 图片粘贴、拖放与资源管理 | 多页签路径/图片链接随文件树操作一致变更 | T37、T39～T40、T44～T45 | 进行中 | 部分覆盖 | 活动页签图片链路已由 runtime 保持；T39 已生成全部已加载文档的图片链接改写预案，实际安全提交仍待 T40 |
...
| 必须实现 | R13 | 多文档页签与最近关闭文档 | 完整实现真实页签、排序、批量关闭、最近关闭和会话恢复 | T35～T46 | 进行中 | 一致 | T35～T39 已完成模型、仓储、路径身份、P1 真实 runtime、可见交互、最近关闭重开和当前会话元数据持久化；批量动作、已有会话启动恢复和阶段验收仍待后续任务 |
...
- 实际落地情况：已新增 `WorkspaceTabBar`、共享 `TabMenu`、`TabOverflowMenu` 与 `TabContextMenu`，由 P1 直接消费 `WorkspaceTabManager` 快照。页签条显示活动项、同名父路径提示和由 T35 公共优先级投影的 unloaded/loading/dirty/saving/readonly/conflict/missing/error 等状态；图形、文字与可访问名称共同表达，不依赖颜色。
  ...
  - 单页签关闭复用现有 manager/controller 结算：关闭前提交活动 adapter 投影，安全关闭后再切换树选择、窗口标题和活动 runtime；阻塞时保持页签并给出真实原因。关闭其他/右侧/全部在 T40 两阶段结算前保持禁用并显示原因，不伪装成功；最近关闭入口与顺序持久化仍归 T39。
...
- 实际落地情况：已扩展 `WorkspaceTabManager` 与页签 reducer，最近关闭项通过统一 `open()` 重新请求 Rust 路径身份后才恢复；失败项留在列表并显示稳定错误，可显式移除最近记录且不删除磁盘文件。`WorkspaceTabBar` 的溢出菜单复用 `TabMenu`，在全部打开页签之后展示最近关闭区，成功重开后恢复可兼容的模式、选择和锚点。
  ...
  - 新增纯 `tabPathImpact` 分析器，rename/move/delete 在磁盘调用前收集全部受影响页签、目标相对路径及所有已加载文档的内联图片链接改写预案。T40 尚未具备两阶段结算和批量 runtime 重映射时，任何命中打开页签的破坏性操作均在 Rust 磁盘命令之前被真实阻止；未命中的文件操作仍沿用原磁盘先行链路，失败不改树或页签。
