...
- 2026-07-27 已在 T41 当前本地状态重新执行门禁：Node 独立回归 30/30、Vitest 32 个文件 254/254、Rust 201 项通过且 1 项手动性能探针忽略、类型检查、生产构建、Rust fmt/全 feature Clippy 和许可证 727/511/0 通过；页签专项仍为 63/63，macOS Tauri/WebKit 桌面套件 11/11 通过。T41 新增测试覆盖三值打开偏好、向后兼容、保存/重置/失败回滚、P1/P2 双入口，以及 mixed 页签集合对替换/退出 intent 的取消与最终提交；既有真桌面套件证明 P1/P2、批量结算和磁盘链路未回退，但没有新增 T41 窗口事务 E2E，完整双平台验收仍属 T45。各任务留痕中的测试数字只是当时的历史快照。最新远端证据仍为 GitHub Actions run `30082725332` 在 macOS/Windows 通过第二阶段 9 条套件，尚未覆盖第三阶段本地提交。
...
- 实际落地情况：已在 `plainroot-preferences-v1.json` 的 schema v1 中增加向后兼容的可选全局 `workspaceOpenDisposition`，Rust/TypeScript 共同约束 `ask/current_window/new_window` 三值；缺失字段默认 `ask`，保存采用既有原子仓储，写失败保持磁盘与内存旧值，并提供读取、保存、重置命令。
  ...
  - P1 已将 Rust 的 `replace_workspace/close_window/quit_app` intent 映射到 T40 的同一不可变全页签结算批次。任一 dirty/save_failed/readonly/conflict 未解决时阻止最终动作；取消会显式向 Rust 拒绝 intent，全部安全后先持久化当前内容无关页签会话，再允许 Rust 提交替换、关闭或整组退出。最终提交失败保留页签和批次，可重试或取消，不虚报窗口事务已回滚。
  ...
  - 本地验证通过：254/254 Vitest、201/201 Rust（另 1 项手动性能探针忽略）、30/30 Node 独立回归、63/63 页签专项、类型检查、生产构建、Rust fmt/全 feature Clippy、许可证 727/511/0 和 macOS 真桌面 E2E 11/11。浏览器对 P2 设置入口执行了 1100×760 与 740×760 两档实测，无根级横向溢出，弹层可见且取消后焦点返回触发器。Windows、远端 CI、真实双窗口退出/替换桌面 E2E 和原生设置菜单人工验收未在本任务执行，留给 T45～T46。完整证据见 `t41-window-lifecycle.md`。
