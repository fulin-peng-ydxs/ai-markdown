...
- 2026-07-25 已在当前 HEAD `9906a6f581fcc4e16d6f81dc06e16f31df528a3f` 重新执行完整 `pnpm test`：Node 独立回归 30/30、Vitest 23 个文件 176/176、Rust 180 项通过且 1 项手动性能探针忽略。首次运行因非交互 shell 的 `PATH` 未包含已安装 Cargo 而在 Rust 前中断；补入用户级 Cargo 可执行目录后同一完整命令通过。T32 验收记录中的 176 Rust/173 Vitest 是 T32 当时的历史快照；T33/T34 新增用例后的当前数字由 `t33-directory-image-move-warning.md`、`t34-editor-chrome-ux-refinement.md` 和本次实跑共同佐证。当前 9 条隔离桌面 E2E 的最新远端证据仍为 GitHub Actions run `30082725332` 在 macOS/Windows 完整通过。
...
| R3 | 每页签独立消费现有排版编辑与统一 history | 待开始 | 部分覆盖 | T35、T37、T39、T44～T45 | 多页签编辑、切换、撤销栈和活动 editor 唯一性测试 |
...
| R10 | 每页签独立保留排版/源码模式和源码选择 | 待开始 | 部分覆盖 | T35、T37、T42、T44～T45 | 页签切换/重启后的模式与选择恢复 |
| R11 | 页签加载、保存、冲突、缺失、恢复和批量结算状态真实可见 | 待开始 | 部分覆盖 | T38～T46 | 状态优先级、文案、失败内容安全与页面测试 |
| R12 | 阶段 7 实现工作区全文搜索 | 跳过 | 未覆盖 | - | 搜索菜单继续禁用 |
| R13 | 完整纳入：真实页签、唯一打开、排序、批量关闭、溢出、最近关闭、独立会话和恢复 | 待开始 | 一致 | T35～T46 | 状态机、组件、服务、桌面 E2E 和阶段验收 |
...
| 必须实现 | R3 | 所见即所得 Markdown 编辑 | 每页签独立消费既有 DocumentSession/editor | T35、T37、T39、T44～T45 | 待开始 | 部分覆盖 | 不重写第二阶段编辑器 |
...
| 必须实现 | R10 | Markdown 源码模式 | 每页签独立保留 visual/source 模式、选择与锚点 | T35、T37、T42、T44～T45 | 待开始 | 部分覆盖 | 复用第二阶段 CodeMirror |
| 必须实现 | R11 | 操作与文件状态反馈 | 实现页签、结算、恢复和持久化新增状态 | T38～T46 | 待开始 | 部分覆盖 | 仅页签相关子集；完整验收仍属阶段 4 |
| 必须实现 | R12 | 工作区全文搜索 | 暂不纳入 | - | 跳过 | 未覆盖 | 阶段 7 实现；页签列表不冒充搜索 |
| 必须实现 | R13 | 多文档页签与最近关闭文档 | 完整实现真实页签、排序、批量关闭、最近关闭和会话恢复 | T35～T46 | 待开始 | 一致 | 第三阶段主体 |
...
| 风险 | 影响 | 缓解与验证 |
| --- | --- | --- |
| 多个已加载大文档使内存增长 | 切换卡顿或系统内存压力 | 只挂载活动 adapter；恢复项 lazy load；T35/T45 实测并报告，不静默丢 history |
