...
>
> 计划状态：执行中（T35～T40 已完成；T41～T46 未实施）
>
...
- 2026-07-26 已在 T40 评审整改后的当前本地状态重新执行门禁：Node 独立回归 30/30、Vitest 31 个文件 249/249、Rust 200 项通过且 1 项手动性能探针忽略、类型检查、生产构建、Rust fmt/全 feature Clippy 和许可证 727/511/0 通过；T40 页签专项为 63/63，macOS Tauri/WebKit 桌面套件 11/11 通过。新增真实桌面用例覆盖关闭右侧及打开页签的改名、移动、图片链接写回和删除，磁盘成功后才提交页签状态。各任务留痕中的测试数字只是当时的历史快照。最新远端证据仍为 GitHub Actions run `30082725332` 在 macOS/Windows 通过第二阶段 9 条套件，尚未覆盖第三阶段本地提交。
...
| R5 | 将保存、恢复、冲突和关闭保护扩展为全页签结算 | 进行中 | 部分覆盖 | T37、T40～T46 | T40 已完成不可变批次、逐项重试/冲突/另存/放弃和最终一次性移除；窗口替换/退出消费该能力仍待 T41，阶段回归待 T44～T46 |
...
| R30 | 纳入页签切换/关闭、重新打开、关闭窗口和弹层焦点子集 | 进行中 | 部分覆盖 | T38～T46 | T40 已让页签上下文菜单消费真实批量关闭并复用 AppDialog 的 Esc/焦点返回；原生快捷键和窗口级流程仍待 T41/T43～T46 |
...
| 必须实现 | R5 | 自动保存、恢复和冲突 | 把单文档保护扩展为关闭、替换、退出的全页签结算 | T37、T40～T46 | 进行中 | 部分覆盖 | T40 已完成逐项决策与一次性页签提交；窗口替换/退出接线和阶段验收待 T41、T44～T46 |
