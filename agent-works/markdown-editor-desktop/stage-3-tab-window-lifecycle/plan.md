...
>
> 计划状态：执行中（T35～T44 已完成；T45～T46 未实施）
>
...
- 2026-07-27 已在 T44 当前本地状态以 `pnpm verify:non-desktop` 从头执行统一门禁：Node 独立回归 30/30、Vitest 32 个文件 267/267、Rust no-default/all-features 均 204 项通过且 1 项手动性能探针忽略、类型检查、生产构建、Rust fmt、全 target/all-features Clippy 和许可证 727/511/0 通过；`test:stage-3:contracts` 另以 110/110 Vitest 加完整 Rust 服务测试固化页签/工作台/仓储/窗口/菜单专项。tagged-union parity 已按 `kind` 逐变体比较字段，反向测试证明字段放错变体会失败。macOS Tauri/WebKit 11/11 与原生菜单点击、`Cmd+W`、`Cmd+Shift+T` 仍是 T43 当前代码证据；T44 无页面变化，未重复冒充桌面证据。`Ctrl+Tab` 真实系统输入、Windows 和远端 CI 仍由 T45 验证。各任务留痕中的测试数字只是当时的历史快照；最新远端证据仍为 GitHub Actions run `30082725332` 在 macOS/Windows 通过第二阶段 9 条套件，尚未覆盖第三阶段本地提交。
...
- 原生菜单已将 `Cmd/Ctrl+Shift+W` 用于关闭窗口，并由 T43 启用 `Cmd/Ctrl+W` 关闭当前页签、`Cmd/Ctrl+Shift+T` 重开最近关闭、`Ctrl+Tab` / `Ctrl+Shift+Tab` 切换页签及批量页签命令；React 不注册会与原生加速键双触发的全局 keydown。
...
| --- | --- | --- | --- | --- | --- |
| R1 | 在现有 Windows/macOS 壳内增加真实多页签和会话恢复，不重做安装/授权 | 进行中 | 部分覆盖 | T41～T46 | T42 已在既有壳接入页签会话恢复；真实重启桌面 E2E 与双平台验收仍待 T45～T46 |
| R2 | 文件树打开、重命名、移动、删除同步作用于全部相关页签 | 进行中 | 部分覆盖 | T37、T39～T40、T44～T45 | T40 已解除 T39 的临时阻断：命中打开页签时先安全结算，磁盘成功后一次提交全部 runtime 路径/身份和树状态；T44 非桌面回归通过，真实桌面阶段回归仍待 T45 |
| R3 | 每页签独立消费现有排版编辑与统一 history | 进行中 | 部分覆盖 | T35、T37、T39、T44～T45 | T37 已完成每页签独立 session/history 与活动 editor 单挂载，T44 已纳入统一非桌面门禁；真实桌面回归仍待 T45 |
| R4 | 阶段 6 实现大纲；本阶段只保证活动页签单一来源可供未来消费 | 跳过 | 未覆盖 | - | 确认无假大纲入口 |
| R5 | 将保存、恢复、冲突和关闭保护扩展为全页签结算 | 进行中 | 部分覆盖 | T37、T40～T46 | T40 已完成不可变批次和一次性提交，T41 已让窗口替换、关闭和退出消费同一批次，T42 恢复活动页签后继续复用第二阶段恢复快照链；T44 非桌面故障矩阵通过，桌面/阶段验收仍待 T45～T46 |
| R6 | 保持每页签图片链路；目录/文件移动时处理全部相关打开文档的链接和状态 | 进行中 | 部分覆盖 | T37、T39～T40、T44～T45 | T40 已在路径提交后对全部受影响已加载页签重新计算内联图片链接并进入各自保存链；T44 非桌面回归通过，真实磁盘桌面回归仍待 T45 |
...
| R10 | 每页签独立保留排版/源码模式和源码选择 | 进行中 | 部分覆盖 | T35、T37、T42、T44～T45 | T42 已恢复各页签模式、选择和锚点元数据，并在首次激活时加载真实内容；真实重启桌面回归仍待 T45 |
| R11 | 页签加载、保存、冲突、缺失、恢复和批量结算状态真实可见 | 进行中 | 部分覆盖 | T38～T46 | T43 已让原生菜单启用态消费聚焦窗口真实页签状态，T44 已固化非桌面状态/契约门禁；桌面矩阵与阶段验收仍待 T45～T46 |
| R12 | 阶段 7 实现工作区全文搜索 | 跳过 | 未覆盖 | - | 搜索菜单继续禁用 |
| R13 | 完整纳入：真实页签、唯一打开、排序、批量关闭、溢出、最近关闭、独立会话和恢复 | 进行中 | 一致 | T35～T46 | T44 已将页签、工作台、仓储、窗口和菜单纳入统一非桌面门禁；真实重启 E2E 和阶段验收仍待 T45～T46 |
| R14 | 纳入页签会话、打开偏好、全页签替换保护和独立窗口恢复；位置/尺寸/三栏布局留阶段 4 | 进行中 | 部分覆盖 | T36、T40～T46 | T42 已消费根会话引用并恢复独立窗口页签集合，单项失败不阻断其他窗口；真实多窗口重启 E2E 与双平台验收仍待 T45～T46 |
| R15 | 阶段 5 实现主题工作室 | 跳过 | 未覆盖 | - | P3 不注册 |
| R30 | 纳入页签切换/关闭、重新打开、关闭窗口和弹层焦点子集 | 进行中 | 部分覆盖 | T38～T46 | T43 已完成原生页签菜单与聚焦窗口命令路由，macOS 已实测菜单点击、`Cmd+W` 与 `Cmd+Shift+T`；`Ctrl+Tab` 真实注入、Windows 和阶段验收仍待 T45～T46 |
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| 必须实现 | R1 | Windows/macOS 本地优先桌面应用 | 在既有桌面壳内接入多页签、窗口会话和双平台回归 | T41～T46 | 进行中 | 部分覆盖 | T42 已接入页签会话恢复；真实重启 E2E 和双平台回归待 T45～T46 |
| 必须实现 | R2 | 文件与目录管理 | 文件树入口、CRUD 与全部相关页签联动 | T37、T39～T40、T44～T45 | 进行中 | 部分覆盖 | T40 已在磁盘成功后批量提交路径/runtime 或页签移除，T44 非桌面回归通过；真实桌面回归待 T45 |
| 必须实现 | R3 | 所见即所得 Markdown 编辑 | 每页签独立消费既有 DocumentSession/editor | T35、T37、T39、T44～T45 | 进行中 | 部分覆盖 | 每页签独立 session/history 与活动 editor 单挂载已纳入 T44 统一门禁；桌面/阶段验收未完成 |
| 必须实现 | R4 | 当前文档大纲 | 暂不纳入 | - | 跳过 | 未覆盖 | 阶段 6 实现；活动页签只预留单一来源 |
| 必须实现 | R5 | 自动保存、恢复和冲突 | 把单文档保护扩展为关闭、替换、退出的全页签结算 | T37、T40～T46 | 进行中 | 部分覆盖 | 恢复与全页签结算已通过 T44 非桌面故障矩阵；真实窗口/重启桌面回归和阶段验收待 T45～T46 |
| 必须实现 | R6 | 图片粘贴、拖放与资源管理 | 多页签路径/图片链接随文件树操作一致变更 | T37、T39～T40、T44～T45 | 进行中 | 部分覆盖 | 全部受影响已加载页签的链接重算与失败保持 dirty 已纳入 T44 门禁；真实磁盘桌面回归待 T45 |
...
| 必须实现 | R10 | Markdown 源码模式 | 每页签独立保留 visual/source 模式、选择与锚点 | T35、T37、T42、T44～T45 | 进行中 | 部分覆盖 | T42 已恢复模式、选择与锚点元数据并在首次激活读取正文；真实重启 E2E 待 T45 |
| 必须实现 | R11 | 操作与文件状态反馈 | 实现页签、结算、恢复和持久化新增状态 | T38～T46 | 进行中 | 部分覆盖 | 页签/结算/恢复/菜单状态已纳入 T44 非桌面门禁；完整验收仍属阶段 4 |
| 必须实现 | R12 | 工作区全文搜索 | 暂不纳入 | - | 跳过 | 未覆盖 | 阶段 7 实现；页签列表不冒充搜索 |
| 必须实现 | R13 | 多文档页签与最近关闭文档 | 完整实现真实页签、排序、批量关闭、最近关闭和会话恢复 | T35～T46 | 进行中 | 一致 | T44 已固化页签/工作台/仓储/窗口/菜单统一门禁；真实重启/多窗口 E2E 与阶段验收待 T45～T46 |
| 必须实现 | R14 | 一目录一窗口与多窗口生命周期 | 页签会话、偏好、全页签替换保护和独立窗口恢复 | T36、T40～T46 | 进行中 | 部分覆盖 | T42 已按真实根会话引用恢复独立窗口页签；位置/尺寸/三栏布局仍属阶段 4，真实多窗口重启回归待 T45 |
| 必须实现 | R15 | 颜色主题预设与实时预览 | 暂不纳入 | - | 跳过 | 未覆盖 | 阶段 5 实现，P3 不注册 |
| 必须实现 | R30 | 核心命令键盘操作 | 页签切换/关闭/重开、关闭窗口与弹层焦点子集 | T38～T46 | 进行中 | 部分覆盖 | T43 已完成原生页签菜单和聚焦窗口路由，macOS 实测 `Cmd+W`/`Cmd+Shift+T`；`Ctrl+Tab` 真实输入、Windows、搜索、阅读和布局命令留对应任务/阶段 |
