...
>
> 计划状态：执行中（T35～T44 已完成；T45 本地实现与 macOS 验证已完成、远端双平台证据待补；T46 未实施）
>
...
| --- | --- | --- | --- | --- | --- |
| R1 | 在现有 Windows/macOS 壳内增加真实多页签和会话恢复，不重做安装/授权 | 进行中 | 部分覆盖 | T41～T46 | T45 已在 macOS 真实重启桌面进程验证会话恢复与单项失败隔离；Windows、远端双平台和阶段验收仍待 T45～T46 |
| R2 | 文件树打开、重命名、移动、删除同步作用于全部相关页签 | 进行中 | 部分覆盖 | T37、T39～T40、T44～T45 | T40 已解除 T39 的临时阻断：命中打开页签时先安全结算，磁盘成功后一次提交全部 runtime 路径/身份和树状态；T45 macOS 主链已回归真实改名、移动、删除及图片链接写回，Windows/远端仍待验证 |
| R3 | 每页签独立消费现有排版编辑与统一 history | 进行中 | 部分覆盖 | T35、T37、T39、T44～T45 | T37 已完成每页签独立 session/history 与活动 editor 单挂载，T45 macOS 桌面链已回归多页签编辑与单 adapter；Windows/远端仍待验证 |
| R4 | 阶段 6 实现大纲；本阶段只保证活动页签单一来源可供未来消费 | 跳过 | 未覆盖 | - | 确认无假大纲入口 |
| R5 | 将保存、恢复、冲突和关闭保护扩展为全页签结算 | 进行中 | 部分覆盖 | T37、T40～T46 | T40 已完成不可变批次和一次性提交，T41 已让窗口替换、关闭和退出消费同一批次，T42 恢复活动页签后继续复用第二阶段恢复快照链；T45 macOS 已验证真实替换拒绝、跨进程恢复和既有保存/冲突桌面主链，Windows/远端与 T46 阶段验收仍待完成 |
| R6 | 保持每页签图片链路；目录/文件移动时处理全部相关打开文档的链接和状态 | 进行中 | 部分覆盖 | T37、T39～T40、T44～T45 | T40 已在路径提交后对全部受影响已加载页签重新计算内联图片链接并进入各自保存链；T45 macOS 主链已回归真实磁盘移动与图片链接写回，Windows/远端仍待验证 |
...
| R10 | 每页签独立保留排版/源码模式和源码选择 | 进行中 | 部分覆盖 | T35、T37、T42、T44～T45 | T42 已恢复各页签模式、选择和锚点元数据，并在首次激活时加载真实内容；T45 macOS 跨进程恢复与惰性激活已通过，Windows 待远端验证 |
| R11 | 页签加载、保存、冲突、缺失、恢复和批量结算状态真实可见 | 进行中 | 部分覆盖 | T38～T46 | T43 已让原生菜单启用态消费聚焦窗口真实页签状态，T44 已固化非桌面状态/契约门禁；T45 macOS 已验证 1100/1050/820/760/740 px 桌面矩阵与恢复缺失项反馈，完整验收仍属阶段 4 |
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| 必须实现 | R1 | Windows/macOS 本地优先桌面应用 | 在既有桌面壳内接入多页签、窗口会话和双平台回归 | T41～T46 | 进行中 | 部分覆盖 | T45 已在 macOS 真实重启桌面进程验证会话恢复与单项失败隔离；Windows、远端双平台和阶段验收仍待 T45～T46 |
| 必须实现 | R2 | 文件与目录管理 | 文件树入口、CRUD 与全部相关页签联动 | T37、T39～T40、T44～T45 | 进行中 | 部分覆盖 | T40 已在磁盘成功后批量提交路径/runtime 或页签移除；T45 macOS 主链已回归真实改名、移动、删除与图片链接写回，Windows/远端仍待验证 |
| 必须实现 | R3 | 所见即所得 Markdown 编辑 | 每页签独立消费既有 DocumentSession/editor | T35、T37、T39、T44～T45 | 进行中 | 部分覆盖 | 每页签独立 session/history 与活动 editor 单挂载已纳入 T44 统一门禁，T45 macOS 已回归真实多页签编辑与单 adapter；Windows/远端与阶段验收未完成 |
| 必须实现 | R4 | 当前文档大纲 | 暂不纳入 | - | 跳过 | 未覆盖 | 阶段 6 实现；活动页签只预留单一来源 |
| 必须实现 | R5 | 自动保存、恢复和冲突 | 把单文档保护扩展为关闭、替换、退出的全页签结算 | T37、T40～T46 | 进行中 | 部分覆盖 | 恢复与全页签结算已通过 T44 非桌面故障矩阵；T45 macOS 已验证真实替换拒绝、跨进程恢复和既有保存/冲突主链，Windows/远端与阶段验收待完成 |
| 必须实现 | R6 | 图片粘贴、拖放与资源管理 | 多页签路径/图片链接随文件树操作一致变更 | T37、T39～T40、T44～T45 | 进行中 | 部分覆盖 | 全部受影响已加载页签的链接重算与失败保持 dirty 已纳入 T44 门禁；T45 macOS 主链已回归真实磁盘移动和图片链接写回，Windows/远端仍待验证 |
...
| 必须实现 | R10 | Markdown 源码模式 | 每页签独立保留 visual/source 模式、选择与锚点 | T35、T37、T42、T44～T45 | 进行中 | 部分覆盖 | T42 已恢复模式、选择与锚点元数据并在首次激活读取正文；T45 macOS 跨进程恢复与惰性激活已通过，Windows/远端仍待验证 |
| 必须实现 | R11 | 操作与文件状态反馈 | 实现页签、结算、恢复和持久化新增状态 | T38～T46 | 进行中 | 部分覆盖 | 页签/结算/恢复/菜单状态已纳入 T44 非桌面门禁；T45 macOS 已验证桌面矩阵和恢复缺失项反馈，完整验收仍属阶段 4 |
...
- 状态：进行中（本地实现与 macOS 验证完成；Windows/远端 CI/artifact 待推送后核验）。
- 依赖：T44。
...
- 实际落地情况：
  ...
  - macOS 以 `osascript`/System Events 定位唯一 fixture 窗口，先等待目标进程成为前台且对应原生菜单项已启用，再发送真实系统键盘事件；每个方向只发送一次、不做业务重试。`Cmd+Shift+]` 可触发但 `Cmd+Shift+[` 在最新 Tauri 二进制中不可触发，因此产品映射收敛为已实测双向有效的 `Cmd+Option+Right/Left`；Windows 继续使用 `Ctrl+Tab` / `Ctrl+Shift+Tab`，等待 runner 实证。
  - 页面矩阵在真实 P1/P2 中覆盖 1100/1050/820/760/740 px，不产生根级横向溢出；主命令、状态路径、页签条与其滚动归属保持可见。确定性流程不配置测试重试；每段失败立即停止后续阶段。
  - 本地 macOS 已在最新 E2E release 二进制上通过 15/15 桌面用例；`pnpm verify:non-desktop` 同时通过 30/30 Node、267/267 Vitest、205 项 no-default Rust与 205 项 all-features Rust（均另 1 项手动探针忽略）、fmt、Clippy、typecheck、生产构建和许可证 727/511/0。Windows 编译/桌面 E2E、最新 HEAD 双平台 CI、run URL 与 artifact 摘要尚未取得，因此 T45 仍为进行中，不开始 T46。完整证据与未验证项见 `t45-tab-desktop-e2e.md`。
