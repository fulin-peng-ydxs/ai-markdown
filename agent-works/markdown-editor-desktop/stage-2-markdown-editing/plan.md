...
- T18 已把 Milkdown 与 CodeMirror 聚合依赖精确写入清单与锁文件；T24 为避免 `basicSetup` 隐式带入第二套 history，又把已锁定的 `@codemirror/commands@6.10.4`、`language@6.12.4`、`search@6.7.1`、`state@6.7.1`、`view@6.43.6` 提升为显式直接依赖。完整依赖图已由仓库许可证门禁验证为 727 个 Node 包、508 个 Rust 包、0 个阻断项。
...
- 第一阶段测试基线为 115 个 Rust 单测、3 个路径工具测试、18 个树状态测试、4 个永久删除反馈测试、35 个 React 测试、1 个 fixture 测试、4 个许可证测试和 4 个桌面 E2E。T18 新增 7 个编辑器 PoC 测试，T19 新增 28 个 session/history/AST 契约测试与 1 个 P1 陈旧读取回归，T20 及其整改新增 21 个恢复仓储/契约测试，T21 新增 16 个冲突/另存/契约测试，T22 新增 15 个资源偏好、导入和 raw IPC 契约测试，T23 新增 22 个 Milkdown adapter/组件/性能与安全策略测试，T24 新增 15 个 CodeMirror adapter/组件/原始换行投影测试；当前 Rust 全量为 167 项，`pnpm test` 的 Vitest 汇总为 108 项。第二阶段不得删除或弱化这些基线来换取绿灯。
...
| R10 | Markdown 源码模式 | 完整纳入本阶段 | 进行中 | T18、T19、T24、T25、T30～T32 | T24 已交付 CodeMirror 正式 adapter、原始换行保真、当前文档查找替换、选择/滚动锚点、只读和 session history 桥接；P1 可见模式切换、系统 IME 与桌面 E2E 仍待 T25/T30～T32 |
| R11 | 操作与文件状态反馈 | 完成本阶段编辑、保存、恢复、只读、冲突和图片异步状态 | 进行中 | T19～T32 | T19 已以 union 表达文档/保存状态，T20～T22 已提供恢复、冲突、另存、资源偏好/上传/导入的稳定错误与安全文案；可见状态组件和产品交互仍待后续任务 |
...
| R30 | 核心命令键盘操作 | 编辑、撤销/重做、保存/另存、模式切换、查找和弹层焦点子集 | 进行中 | T23～T25、T27～T32 | T23/T24 已把两种编辑器的 Cmd/Ctrl+Z/重做桥接到统一 session，并完成源码内查找替换；菜单、保存、模式和双平台快捷键仍待后续任务 |
| R31 | 无障碍基础 | 编辑器、保存状态、冲突/恢复/资源弹层子集 | 进行中 | T23～T25、T27～T32 | T23/T24 已提供排版/源码 textbox、toolbar/查找语义、可见焦点、只读和非颜色反馈；完整键盘遍历、对比度与系统辅助技术仍待后续任务 |
...
| 必须实现 | R3 | 所见即所得编辑 | Milkdown、统一模型、格式化、粘贴和往返 | T18、T19、T23、T25、T30～T32 | 进行中 | 部分覆盖 | T23 已完成真实 Milkdown AST parser、排版 adapter 与组件；正式 P1 编辑器、系统 IME 和桌面验收仍待 T25/T30～T32 |
...
| 必须实现 | R10 | 源码模式 | CodeMirror、统一内容源、异常语法保留 | T18、T19、T24、T25、T30～T32 | 进行中 | 部分覆盖 | T24 已完成正式 CodeMirror adapter、异常语法和原始换行保留、当前文档查找替换与统一 history 桥接；可见模式切换、系统 IME 和桌面验收仍待 T25/T30～T32 |
| 必须实现 | R11 | 真实反馈 | 编辑/保存/恢复/冲突/图片全状态 | T19～T32 | 进行中 | 部分覆盖 | T19 已建立可枚举文档/保存/内容安全状态，T20～T22 已提供恢复、冲突、另存和资源导入的稳定失败契约；可见反馈和完整异步消费者仍待后续任务 |
...
| 必须实现 | R30 | 键盘流程 | 编辑/保存/模式/当前文档查找/弹层焦点 | T23～T25、T27～T32 | 进行中 | 部分覆盖 | T23/T24 已桥接两种 adapter 的统一撤销重做，并完成源码内查找替换键盘入口；保存、模式、弹层及页签/阅读/工作区搜索仍待对应阶段 |
| 必须实现 | R31 | 无障碍 | 编辑器与本阶段弹层/状态子集 | T23～T25、T27～T32 | 进行中 | 部分覆盖 | T23/T24 已提供排版/源码 textbox、可见焦点、只读与中文查找语义；P1 完整键盘链、弹层和跨平台辅助技术仍待后续任务 |
...
- 具体完成情况：T19 已覆盖 session/history；T20 已覆盖恢复仓储；T21 已新增 16 个冲突/另存/契约测试；T22 已新增 15 个偏好、图片资源和 raw IPC 契约测试。自动保存、editor adapter 及页面状态仍待后续任务。
