...
- 实际落地情况：已新增 `ConflictDialog`、`RecoveryDialog`、`SaveCopyDialog` 与共享 `ContentSafetySummary`，全部组合既有 `AppDialog`、`AsyncStatePanel`、`plainroot-button` 和语义 token。冲突弹层展示相对路径、磁盘修改时间/大小、当前 session 状态和内容安全位置；“准备覆盖磁盘版本”会重新取得一次性证据，并以“覆盖磁盘版本”完成第二次确认，令牌过期、磁盘再次变化或内容变化均保留当前内容并回到可重试选择。
  ...
  - 组件、session 与 P1/P2 接线新增 14 项 Vitest，当前总计 150/150；Rust 171 项通过、1 项手动探针忽略。类型、Vite 生产构建、fmt、Clippy、许可证 727/508/0 和 Node 独立门禁通过。浏览器临时验证页实际检查冲突四选项、覆盖二次确认和恢复弹层，发现并修复共享 dialog 宽度下的横向裁切；临时验证文件已删除。未运行真实 Tauri 原生保存选择器、桌面 E2E、Windows 或系统辅助技术，仍由 T29～T31 承接。证据见 `t27-recovery-conflict-ui.md`。
