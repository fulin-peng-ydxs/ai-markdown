...
- 第一阶段测试基线为 115 个 Rust 单测、3 个路径工具测试、18 个树状态测试、4 个永久删除反馈测试、35 个 React 测试、1 个 fixture 测试、4 个许可证测试和 4 个桌面 E2E。T18 新增 7 个编辑器 PoC 测试，T19 新增 28 个 session/history/AST 契约测试与 1 个 P1 陈旧读取回归，T20 及其整改新增 21 个恢复仓储/契约测试，T21 新增 16 个冲突/另存/契约测试，T22 新增 15 个资源偏好、导入和 raw IPC 契约测试，T23 新增 22 个 Milkdown adapter/组件/性能与安全策略测试，T24 新增 15 个 CodeMirror adapter/组件/原始换行投影测试，T25 新增生产 AST 兼容性、统一编辑壳、只读投影和 P1 降级回归，T26 新增保存控制器、窗口结算契约和工作台接线回归；当前 Rust 全量为 171 项通过、1 项手动性能探针忽略，Vitest 汇总为 136 项。第二阶段不得删除或弱化这些基线来换取绿灯。

### 2.2 已有代码与可复用能力

- `src/features/workbench/WorkspaceWorkbench.tsx` 当前已接入文件树、读取、CRUD、watch、窗口决策、真实 `DocumentEditorShell` 和 `DocumentSaveController`，并消费单一 `DocumentSession`、生产 Remark/GFM 兼容性解析、文档级陈旧读取保护、排版/源码 adapter、恢复快照服务与窗口结算意图；仍没有恢复/冲突可见流程或图片输入消费者。
...
| R5 | 自动保存、恢复与外部冲突 | 完成单文档链路；多页签关闭检查由阶段 3 扩展，搜索索引异常协同由阶段 7 承接 | 进行中 | T19～T21、T25～T27、T29～T32 | T19 已落地保存 union，T20 已落地恢复仓储，T21 已落地冲突证据/一次性覆盖与原生单目标另存后端；T26 已接入尺寸分级自动保存、手动保存、恢复快照和关闭/替换/退出结算，恢复/冲突可见 UI 仍待 T27 |
...
| R11 | 操作与文件状态反馈 | 完成本阶段编辑、保存、恢复、只读、冲突和图片异步状态 | 进行中 | T19～T32 | T26 已让 P1 的 dirty/saving/saved/save_failed/conflict 与窗口结算阻塞状态来自真实 session/磁盘结果，并提供真实手动保存入口；恢复选择、冲突/另存和图片异步消费者仍待 T27～T31 |
...
| R30 | 核心命令键盘操作 | 编辑、撤销/重做、保存/另存、模式切换、查找和弹层焦点子集 | 进行中 | T23～T25、T27～T32 | T25 已让 P1 工具栏消费统一撤销/重做、格式、模式和当前文档查找命令；T26 提供真实手动保存按钮但未提前启用原生保存快捷键，保存/另存菜单、弹层及双平台快捷键仍待 T27～T32 |
| R31 | 无障碍基础 | 编辑器、保存状态、冲突/恢复/资源弹层子集 | 进行中 | T23～T25、T27～T32 | T25 已提供 P1 可见 toolbar、pressed/disabled/readonly 语义、兼容性文字说明、加载回退和焦点可达的两种编辑表面；完整键盘遍历、对比度与系统辅助技术仍待 T27～T32 |
...
| 必须实现 | R5 | 保存/恢复/冲突 | 完整单文档链路和当前窗口门禁 | T19～T21、T25～T27、T29～T32 | 进行中 | 部分覆盖 | T26 已实现尺寸分级自动保存、手动保存、恢复快照触发和当前单文档窗口结算；可见冲突/恢复流程仍待 T27，阶段 3 扩展全部页签关闭检查 |
...
| 必须实现 | R11 | 真实反馈 | 编辑/保存/恢复/冲突/图片全状态 | T19～T32 | 进行中 | 部分覆盖 | T26 已让 P1 呈现真实 dirty/saving/saved/save_failed/conflict 和结算阻塞反馈；恢复选择、冲突/另存与图片异步消费者仍待 T27～T31 |
...
| 必须实现 | R30 | 键盘流程 | 编辑/保存/模式/当前文档查找/弹层焦点 | T23～T25、T27～T32 | 进行中 | 部分覆盖 | T26 已提供真实手动保存按钮但未提前启用原生保存快捷键；保存/另存菜单、弹层及页签/阅读/工作区搜索仍待对应阶段 |
| 必须实现 | R31 | 无障碍 | 编辑器与本阶段弹层/状态子集 | T23～T25、T27～T32 | 进行中 | 部分覆盖 | T25 已提供 P1 可见 toolbar、模式 pressed、disabled/readonly、加载与兼容性文字反馈；弹层、完整键盘链和跨平台辅助技术仍待后续任务 |
...
- 实际落地情况：已新增 `DocumentSaveController`，按 UTF-8 字节数使用 800 ms/2 秒/5 秒自动保存防抖和 2 秒/10 秒恢复快照节流；保存与快照分别单飞，保存中继续编辑会在当前请求结束后追赶最新 `editVersion`，关闭结算等待追赶写完成，不会用新 requestId 覆盖 in-flight 结果。P1 的自动保存、工具栏手动保存、切换当前文件及对当前文件执行重命名/移动/删除均消费同一控制器；只有既有 safe-write 返回新 revision 后才清洁 session 并删除匹配快照，修订冲突保留内存内容和 T21 冲突证据，晚到快照不能在成功保存后复活。
  ...
  - fake timer 与组件测试覆盖尺寸阈值、UTF-8 计量、防抖、单飞/追赶、活动注册前置、快照节流、晚到快照、恢复失败不自旋、冲突、手动保存、切换/关闭结算；Rust 覆盖 close/replace/quit、重复意图、失效窗口清理、拒绝回滚、持久化失败与 Rust↔TypeScript parity。当前验证为 136/136 Vitest、171/171 Rust（另 1 项手动性能探针忽略）、类型检查、fmt、Clippy、前端生产构建和许可证 727/508/0。手动 safe-write 探针在本机 debug 测试路径测得 5/20/64 MiB 约 430/1650/5558 ms，说明 64 MiB 全量安全写仍是明确性能风险；分级防抖、单飞和追赶用于限制写放大，不把该结果表述为大文件保存无延迟。真实 Tauri 系统关闭/退出、桌面 E2E、Windows 运行和强制进程终止仍由 T29～T31 验证。证据见 `t26-save-lifecycle.md`。
...
- 具体完成情况：T19 已覆盖 session/history；T20 已覆盖恢复仓储；T21 已新增 16 个冲突/另存/契约测试；T22 已新增 15 个偏好、图片资源和 raw IPC 契约测试；T23/T24 已覆盖两种 adapter；T25 已覆盖生产 AST 兼容性、模式切换、统一历史、只读投影、陈旧解析拒绝、即时输入提交、空态、查找转源码和 P1 密集结构降级；T26 已覆盖 autosave debounce/single-flight/pending chase、恢复快照限频/失败/晚到清理、手动保存和结算等待。恢复/冲突弹层仍待 T27。
...
- 具体完成情况：T25 已将 visual/source 接入生产 P1，并通过组件测试验证格式、统一撤销/重做、空文档、只读、兼容性降级、加载失败和工作台接线；T26 已增加真实保存按钮、保存状态和窗口结算阻塞反馈，并用组件测试覆盖手动保存与 close intent。真实 Chromium PoC 仍只证明 T25 编辑壳，不是 Tauri P1 IPC 页面；冲突/恢复/图片、系统关闭/退出和 1280/1050/820/740 px 完整 P1 桌面矩阵仍待 T27～T31。
...
- 具体完成情况：T25 已新增并登记 `DocumentEditorShell`、`EditorToolbar`、`SaveStatus` 和 `remarkMarkdownParser`，真实复用 `AsyncStatePanel` 作为延迟加载回退；P1 是编辑壳唯一生产消费者。新增 CSS 只消费既有语义 token，生产 editor/workbench CSS 扫描无私有 hex/rgb/gradient。冲突、恢复、另存与资源弹层仍待 T27/T28。

### 8.5 权限与数据范围测试

- 计划：覆盖前端任意绝对路径注入、根外资源目录、`..`、symlink、save-copy token 重放/换靶、恢复路径伪造、MIME/签名不一致、只读、权限撤销和 app data 不可写；确认 capability 无宽泛文件权限。
- 具体完成情况：T20 已覆盖 recovery workspace 身份绑定；T21 已覆盖另存单目标授权；T22 已覆盖资源目录绝对/`..`/非法名/symlink 拒绝、MIME/签名不一致、SVG/超限、错误 workspace、token 重放/过期/并发和取消前目标变化。Web 输入使用 raw bytes + opaque upload id，确认命令不接收绝对路径。Windows 原生选择和平台运行时仍待 T31。
