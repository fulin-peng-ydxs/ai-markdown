...
>
> 计划状态：已完成本地阶段验收及验收后业务/页面补强（T18～T34 均已完成；已推送的提交 `71cab73070bce27ec59fc8e35c670680d6ee2021` 对应 GitHub Actions run `30063241410` 取得 macOS/Windows 完整 CI 双绿；T33/T34 已取得本地 9/9 真桌面证据，未推送前不宣称其取得新的远端 CI）
>
...
| R3 | 所见即所得 Markdown 编辑 | 完整纳入本阶段 | 已完成 | T18、T19、T23、T25、T30～T32、T34 | 第二阶段范围已验收；macOS/Windows 真实桌面链路已验证排版/源码编辑、保存和重开，T34 本地收敛编辑工作台信息层级，系统输入法候选窗仍待人工验证 |
...
| R6 | 图片粘贴、拖放与资源管理 | 完整纳入本阶段；目录移动增加可能断链提示，不建立跨文档索引 | 已完成 | T18、T22、T23、T28、T30～T34 | 第二阶段范围已验收；两个平台的 WebView `DataTransfer/File` 已贯通上传、确认、Markdown、保存和重开；T33 本地验证目录风险提示，T34 将低频资源设置收敛进资源组，原生选择器、系统剪贴板和 Finder/Explorer 拖入仍是人工项 |
...
| 必须实现 | R3 | 所见即所得编辑 | Milkdown、统一模型、格式化、粘贴、往返与编辑工作台信息层级 | T18、T19、T23、T25、T28、T30～T32、T34 | 已完成 | 部分覆盖 | 第二阶段范围已验收；macOS/Windows 真桌面会话已验证排版/源码连续编辑、保存和重开，T34 本地完成工具栏/状态栏收口，系统 IME 候选窗仍待人工验证 |
...
| 必须实现 | R6 | 图片/资源 | 粘贴、拖放、选择、资源偏好、相对链接、目录移动风险提示与低频入口收敛 | T18、T22、T23、T28、T30～T34 | 已完成 | 部分覆盖 | 第二阶段范围已验收；两个平台 WebView 文件事件已贯通 Rust 上传/确认、相对 Markdown、保存和重开；T33 已在本地验证目录风险提示，T34 将资源设置并入资源组，不建立跨文档引用索引或批量改写 |
...
- 具体完成情况：T30 已新增 `test:editor`、`test:roundtrip`、`test:rust`，并由 `pnpm test` 统一运行 Node、Vitest 和 Rust 非桌面门禁；T34 最新本机结果仍为 Node 30/30、Vitest 176/176、Rust 180 项通过且 1 项手动性能探针忽略。默认全量 `cargo test` 的既有并发 flaky 已以统一临时目录修复并连续 10/10 通过；T31 把第一阶段 P1/P2 基线与第二阶段编辑、资源、保存、外部修改、恢复串成 8 条真桌面用例，T33 增加第 9 条目录图片移动风险检查与取消回归，T34 在该用例组内补充四档编辑 chrome 布局断言。run `30062045288` 已在 macOS/Windows 完整重跑 T31 门禁并双绿；T33/T34 尚未推送，不宣称新远端结果。
...
- 发布前检查：第二阶段仍只生成内部未签名测试构建；T31 代码与后续文档提交已取得远端 macOS/Windows 双绿，T32 文档/需求复核和本地阶段验收已完成，T33/T34 目录图片移动风险与编辑工作台信息层级补强已完成本地门禁和 9/9 真桌面回归但尚未取得新远端结果。正式签名、发布和原生系统人工验收仍不属于本结论。
