...
>
> 计划状态：已完成当前阶段验收及验收后业务/页面补强（T18～T34 均已完成；提交 `914ad8413b30569ab1c704dc1a55f15d3ed78c59` 对应 GitHub Actions run `30082725332` 已在 macOS/Windows 完成当前非桌面门禁、9/9 桌面 E2E、未签名生产构建和 artifact 上传）
>
...
- 当前实现架构：`agent-works/markdown-editor-desktop/architecture/desktop-foundation.md`、`agent-works/markdown-editor-desktop/architecture/markdown-document-editing.md`。
...
- 第一阶段测试基线为 115 个 Rust 单测、3 个路径工具测试、18 个树状态测试、4 个永久删除反馈测试、35 个 React 测试、1 个 fixture 测试、4 个许可证测试和 4 个桌面 E2E。T18～T30 已逐步补齐编辑器、session/history、恢复、冲突、另存、资源后端、两种 adapter、保存、图片、页面状态和契约服务门禁；T31～T34 又把桌面 E2E 扩为 9 条真实 Tauri/WebKit/WebView2 用例，并使用每套件独立复制的临时工作区。当前本机门禁为 Rust 180 项通过、1 项手动性能探针忽略，Vitest 176 项、Node 独立回归 30 项和桌面 E2E 9 项通过。第二阶段不得删除、缩小或用重试掩盖这些基线来换取绿灯。
- 当前已推送 HEAD `914ad8413b30569ab1c704dc1a55f15d3ed78c59` 对应远端 run `30082725332`：macOS/Windows 均完成许可证、类型与前端、Rust lint/测试、9/9 桌面 E2E、未签名生产构建和 artifact 上传。T31 首次双绿证据仍在任务 6.14 中作为历史实施记录保留，不作为当前 HEAD 的替代证据。
...
| --- | --- | --- | --- | --- | --- |
| R1 | Windows/macOS 本地优先桌面应用 | 在既有桌面壳内增加离线编辑、保存、恢复和另存；不重做安装/授权 | 已完成 | T18、T29～T32 | 第二阶段范围已验收；当前 macOS/Windows 9/9 真 Tauri/WebKit/WebView2 E2E、生产隔离构建与 artifact 已通过，Windows 原生 UI 保持人工项 |
| R2 | 文件与目录管理 | 复用读取、watch、安全写并接入当前编辑会话、图片资源文件和目录图片移动风险确认；不改变磁盘先行语义 | 已完成 | T19、T21、T22、T26、T28、T30～T31、T33 | 第二阶段范围已验收；两个平台的临时工作区已完成打开、编辑、资源落盘、保存、重开、外部修改、恢复、目录风险提示与取消零副作用回归 |
| R3 | 所见即所得 Markdown 编辑 | 完整纳入本阶段 | 已完成 | T18、T19、T23、T25、T30～T32、T34 | 第二阶段范围已验收；macOS/Windows 真实桌面链路已验证排版/源码编辑、保存、重开和四档编辑工作台信息层级，系统输入法候选窗仍待人工验证 |
...
| R6 | 图片粘贴、拖放与资源管理 | 完整纳入本阶段；目录移动增加可能断链提示，不建立跨文档索引 | 已完成 | T18、T22、T23、T28、T30～T34 | 第二阶段范围已验收；两个平台的 WebView `DataTransfer/File` 已贯通上传、确认、Markdown、保存、重开和目录风险提示，T34 将低频资源设置收敛进资源组；原生选择器、系统剪贴板和 Finder/Explorer 拖入仍是人工项 |
...
| R11 | 操作与文件状态反馈 | 完成本阶段编辑、保存、恢复、只读、冲突、图片和目录移动风险异步状态 | 已完成 | T19～T34 | 第二阶段状态反馈范围已验收；T34 将保存状态收敛为状态栏单一位置并补回当前路径，系统辅助技术和原生系统输入仍待人工验证，后续阶段状态仍未实现 |
...
| R30 | 核心命令键盘操作 | 编辑、撤销/重做、保存/另存、模式切换、查找和弹层焦点子集 | 已完成 | T23～T25、T27～T32、T34 | 第二阶段键盘子集已验收；T34 保持主操作与资源设置的按钮语义和焦点顺序，原生保存/另存快捷键、系统 IME 与 Windows 快捷键仍是人工项，页签/阅读/工作区搜索快捷键待对应阶段 |
| R31 | 无障碍基础 | 编辑器、保存状态、冲突/恢复/资源及移动风险弹层子集 | 已完成 | T23～T25、T27～T34 | 第二阶段无障碍子集已验收；T34 保持可访问按钮名称并用 tooltip 承接完整路径，系统辅助技术、IME 候选窗和 Windows 原生人工证据仍待登记 |
...
- 变更区域：中央只读 `<pre>` 替换为 `DocumentEditorShell`；文档工具层接入排版/源码、保存/另存和资源目录，主操作固定、模式/格式区独立滚动；状态栏单一接入保存、当前路径、模式、字数、编码/换行和光标；新增冲突、恢复与资源对话框。
...
| 7.1.2 排版编辑默认态 | R3、R5、R11、R30、R31 | 输入、格式化、撤销重做、保存；保存状态单一呈现，当前路径持续可见 | Milkdown adapter + 统一 history/save controller；固定主操作区 + 可滚动格式区 | `DocumentEditorShell`、`VisualMarkdownEditor`、`DocumentStatusBar` | session change/save/recovery API | loading/empty/dirty/saving/saved/save_failed/readonly/conflict；长路径省略并保留完整 tooltip | T18、T19、T23、T25～T29、T31、T34 |
...
| 7.1.2 图片与资源 | R2、R6、R11、R30、R31 | 粘贴、拖放、选择图片、配置目录；资源目录移动风险确认；低频目录设置不占顶层文字按钮 | Rust 资源导入 + P1 配置对话框 + editor insertion + 目录风险检查；图片/设置共用资源组 | `AssetDirectoryDialog`、asset service、移动 `AppDialog` | preference/import/confirm/cancel + `inspect_workspace_move_risk` | 重名不覆盖；失败不插入断链；只读禁用；未打开文档只提示风险、不虚称引用命中 | T22～T23、T28、T30～T31、T33～T34 |
| 7.1.2 保存与另存 | R5、R11、R30、R31 | 自动/手动同链路；另存；只读兜底；窄窗主操作固定可见 | safe-write controller + 原生 save target token | `SaveStatus`、`DocumentStatusBar`、`SaveCopyDialog` | safe-write/prepare-save-copy/confirm | 保存失败保留内容；目标覆盖需确认；状态栏是持续保存状态唯一位置 | T20～T21、T26～T27、T29～T31、T34 |
...
| 7.1.3 自适应布局 | R8、R11、R30、R31 | 本阶段只保证编辑器嵌入后不破坏现有退化 | 复用 P1 CSS/抽屉；工具栏主操作固定、格式区内滚；不做分隔条与偏好 | 现有 workbench shell | 无新增布局 API | 1100/1050/820/740 px 无整页或工具栏溢出；完整 R8 未覆盖 | T29～T31、T34 |
...
| 编辑状态 | 复用 `AsyncStatePanel` 的阻塞/加载语义；新增紧凑 `SaveStatus` | 新增 editor status 种类 | `saveState/contentSafety` | 同步状态契约 | 状态栏单一消费，非颜色验证 | T19、T25、T29～T31、T34 |
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| 必须实现 | R1 | 双平台、本地优先 | 既有桌面壳内增加离线编辑/恢复/另存并做双平台回归 | T18、T29～T32 | 已完成 | 部分覆盖 | 第二阶段范围已验收；当前 macOS/Windows 真 Tauri/WebKit/WebView2 9/9 与正式构建隔离通过，Windows 原生人工项尚未完成 |
| 必须实现 | R2 | 文件/目录管理 | 编辑会话接入读取、watch、安全写、资源文件和目录图片移动风险确认 | T19、T21、T22、T26、T28、T30～T31、T33 | 已完成 | 部分覆盖 | 第二阶段范围已验收；两个平台的独立临时工作区已完成读取、图片落盘、保存、重开、外部修改、恢复、目录风险提示和取消零副作用 |
| 必须实现 | R3 | 所见即所得编辑 | Milkdown、统一模型、格式化、粘贴、往返与编辑工作台信息层级 | T18、T19、T23、T25、T28、T30～T32、T34 | 已完成 | 部分覆盖 | 第二阶段范围已验收；macOS/Windows 真桌面会话已验证排版/源码连续编辑、保存、重开和四档工具栏/状态栏收口，系统 IME 候选窗仍待人工验证 |
...
| 必须实现 | R6 | 图片/资源 | 粘贴、拖放、选择、资源偏好、相对链接、目录移动风险提示与低频入口收敛 | T18、T22、T23、T28、T30～T34 | 已完成 | 部分覆盖 | 第二阶段范围已验收；两个平台 WebView 文件事件已贯通 Rust 上传/确认、相对 Markdown、保存、重开和目录风险提示；T34 将资源设置并入资源组，不建立跨文档引用索引或批量改写 |
...
| 必须实现 | R11 | 真实反馈 | 编辑/保存/恢复/冲突/图片及目录移动风险全状态 | T19～T34 | 已完成 | 部分覆盖 | 第二阶段状态反馈范围已验收；T34 消除重复保存状态并补回当前路径，风险提示只陈述可证边界；系统辅助技术与原生系统输入仍待人工验证 |
...
| 必须实现 | R30 | 键盘流程 | 编辑/保存/模式/当前文档查找/弹层焦点 | T23～T25、T27～T32、T34 | 已完成 | 部分覆盖 | 第二阶段键盘子集已验收；T34 保持固定主操作和资源设置的可访问按钮语义，原生保存/另存快捷键、系统 IME 和 Windows 快捷键仍是人工项，页签/阅读/工作区搜索快捷键待对应阶段 |
| 必须实现 | R31 | 无障碍 | 编辑器与本阶段弹层/状态子集 | T23～T25、T27～T34 | 已完成 | 部分覆盖 | 第二阶段无障碍子集已验收；T34 保留资源设置可访问名称和路径 tooltip，系统辅助技术、IME 候选窗和 Windows 原生证据仍待人工登记 |
...
- 实际落地情况：已完成上述实现。Rust 检查同时返回“配置资源目录受影响”“发现受支持图片”“检查是否受限”和最终 `mayBreakImageLinks`，命令仍先校验窗口的当前 workspace 授权；P1 在原移动对话框内增加检查中门禁和风险后果说明，没有新增第二套弹层或状态模型。组件测试锁定风险二次确认、取消零磁盘调用和无风险直接移动；真实 Tauri/WebKit 第 9 条 E2E 通过临时图片验证命令、可见文案、取消和磁盘不变。最终全量回归还暴露 `fs::delete` 的旧时间戳临时目录未消费共享唯一目录工厂，已改为由 `TestDirectory` 持有夹具并向删除服务传入 canonical root，没有修改产品删除语义、放宽路径校验或以重跑掩盖红灯。本机最终通过 Node 30/30、Vitest 176/176、Rust 180 项通过/1 项手动探针忽略、类型检查、生产构建、fmt、全 feature Clippy/tests、许可证 727/508/0、9/9 桌面 E2E 和 `tauri build --no-bundle`；当前 HEAD 的 run `30082725332` 又在 macOS/Windows 复验 9/9 桌面套件、非桌面门禁、生产构建与 artifact 上传。

### 6.17 任务 T34：编辑工具栏与状态栏信息层级收口

- 状态：已完成
- 依赖：T25 统一编辑壳、T28 资源目录入口、T29 持续状态栏、T32 页面验收与用户提供的页面 UX 审查。
- 涉及文件/模块：`src/features/editor/EditorToolbar.tsx`、`DocumentEditorShell.tsx/.css`、`DocumentStatusBar.tsx`、`src/features/workbench/WorkspaceWorkbench.css`、组件测试、桌面 E2E、`DESIGN.md`、需求/计划/留痕与仓库稳定文档。
- 目标：消除保存状态的重复呈现，让窄窗口中的保存、另存和当前文档查找保持可见，并恢复状态栏当前路径，同时降低资源目录低频设置的视觉权重。
- 操作：移除工具栏内的 `SaveStatus` 消费点，保留状态栏唯一持续状态；把模式/历史/格式命令放入独立横向滚动区，把保存、另存、查找与图片资源组固定在右侧；资源目录继续使用原按钮契约但收敛为资源组内带 tooltip/可访问名称的 `⋯`；状态栏显示相对路径并以 tooltip 暴露完整值；空文档占位精简为“开始输入 Markdown”。
- 产出：单一保存状态、固定主操作区、图片资源分组、路径状态项、组件/E2E 回归和 `t34-editor-chrome-ux-refinement.md`。
- 影响范围：只调整 P1 编辑 chrome 的信息层级、响应式滚动归属和文案；不改变 `DocumentSession`、保存/资源命令、磁盘提交、权限、菜单、配置或数据结构。
- 边界与异常：格式区可以横向滚动但不得造成整页/工具栏溢出；保存和查找必须始终在工具栏可视范围；长路径允许省略但完整值必须可查看；资源设置仍调用原 `AssetDirectoryDialog`，不得新增第二套弹出菜单或偏好状态。
- 验证方式：组件测试断言保存状态唯一、路径/tooltip、固定主操作区和资源设置真实入口；生产类型/构建与全量回归；真实 Tauri/WebKit 在 1100/1050/820/740 px 检查整页与工具栏溢出、主操作矩形、保存状态数量和路径。
- 完成标准：同一文档保存状态只出现一次；当前路径回到状态栏；保存/另存/查找不随格式区滚出；低频资源目录不再以长文字顶层按钮常驻；既有编辑、资源和保存链路无回归。
- 实际落地情况：已按上述方案完成，没有新增组件、token、命令或持久配置。`EditorToolbar` 使用两区布局，固定操作区在 1100/1050/820/740 px 均位于工具栏可见边界内，只有模式/历史/格式区保留 `overflow-x: auto`；`SaveStatus` 仅由 `DocumentStatusBar` 消费，状态栏新增相对路径与完整值 tooltip；图片选择和 `⋯` 资源设置共享“图片资源”组，点击仍打开既有 `AssetDirectoryDialog`。组件测试与真实 Tauri E2E 已验证上述契约；全量回归首次运行暴露窗口结算测试错误等待无关的 Milkdown 正文渲染，已改为等待真实 session 保存状态与监听器注册，未用重试或延长超时掩盖。最终通过 Vitest 176/176、Rust 180 项通过/1 项手动探针忽略、类型检查、生产构建、fmt、全 feature Clippy/tests、许可证 727/508/0、9/9 本地桌面 E2E 和 `tauri build --no-bundle`；当前 HEAD 的 run `30082725332` 已在 macOS/Windows 复验包含四档编辑 chrome 断言的 9/9 套件。通知行仍保持内容流布局，因为其错误与兼容性文案可能持续且多行，固定高度会截断安全信息，浮层会遮挡正文，本轮没有以未经像素验证的方案改变该错误承接。
...
- 具体完成情况：T25 已将 visual/source 接入生产 P1，T26 已增加真实保存和窗口结算；T27 已用组件测试覆盖冲突/恢复/另存、P2 根入口、外部删除保护和安全关闭门禁。T28 已覆盖图片流程和 1280/820/740 px 弹层；T29 又验证 1280/1050/820/740 px 与 740×460 低高度工作台、文件树抽屉焦点、状态栏收敛和无根滚动。当前 macOS/Windows runner 已用独立临时工作区完成 P1/P2 真 IPC、两模式编辑、图片、保存、外部修改、恢复、目录风险文案/取消零副作用，以及 1100/1050/820/740 px 下保存/查找固定可见、工具栏与整页无横向溢出、保存状态唯一和路径存在。该 WebView 自动化不替代原生图片输入、系统关闭/退出或系统辅助技术人工验收。
...
- 具体完成情况：T25 已登记编辑壳相关组件；T27 新增并登记 `ConflictDialog`、`RecoveryDialog`、`SaveCopyDialog` 与 `ContentSafetySummary`；T28 新增并登记 `AssetDirectoryDialog`、`workspaceAssetPath` 与 `workspaceImageNodeView`；T29 新增并登记 `DocumentStatusBar`；T34 将 `SaveStatus` 收敛为只由状态栏消费，并更新 `EditorToolbar`/`DocumentStatusBar` 稳定职责。此次复用既有按钮、资源对话框和 token，没有为低频资源设置新增弹出菜单、私有颜色或第二套状态模型；生产 CSS/TSX 扫描除 `tokens.css` 外无色值/渐变命中。
...
- 具体完成情况：T30 已新增 `test:editor`、`test:roundtrip`、`test:rust`，并由 `pnpm test` 统一运行 Node、Vitest 和 Rust 非桌面门禁；当前本机结果为 Node 30/30、Vitest 176/176、Rust 180 项通过且 1 项手动性能探针忽略。默认全量 `cargo test` 的既有并发 flaky 已以统一临时目录修复并连续 10/10 通过；T31 把第一阶段 P1/P2 基线与第二阶段编辑、资源、保存、外部修改、恢复串成 8 条真桌面用例，T33 增加第 9 条目录图片移动风险检查与取消回归，T34 在该用例组内补充四档编辑 chrome 布局断言。run `30082725332` 已在 macOS/Windows 完整重跑当前非桌面门禁、9/9 桌面套件、生产构建与 artifact 上传并双绿。
...
- 具体完成情况：T31 先在本机 macOS 建立 8 条专用 E2E，保留 P2/P1 IPC、布局与焦点基线，并覆盖两模式编辑/资源输入/保存重开、恢复仓储、外部修改内容安全和显式恢复。T33 新增第 9 条真 IPC 用例，在临时工作区创建图片，验证目录风险命令、可见确认文案、取消以及目录/文档仍可读取；T34 在打开 fixture 的真实 P1 中增加 1100/1050/820/740 px 工具栏/状态栏断言。套件使用独立临时工作区、确定性流程不重试；正式构建确认不含 E2E 标记。run `30082725332` 已在 macOS/Windows 完整通过当前许可证、类型与前端、Rust lint/测试、9/9 桌面 E2E、未签名生产构建和 artifact 上传。Windows 原生系统对话框、拖放、剪贴板、菜单和辅助技术继续保留人工未验证。
...
- 需求映射：R1～R34 每项均有任务或跳过原因；T18～T34 在第 3、5、6 章编号一致。
...
- 发布前检查：第二阶段仍只生成内部未签名测试构建；T18～T34 当前 HEAD 已取得远端 macOS/Windows 双绿、9/9 真桌面回归和 artifact 上传。正式签名、发布和原生系统人工验收仍不属于本结论。
- 发布步骤：当前阶段分支已经用户授权推送并取得 macOS/Windows CI 与 artifact；不做签名、公证、自动更新或公开发布。
