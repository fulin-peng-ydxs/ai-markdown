...
- 第一阶段测试基线为 115 个 Rust 单测、3 个路径工具测试、18 个树状态测试、4 个永久删除反馈测试、35 个 React 测试、1 个 fixture 测试、4 个许可证测试和 4 个桌面 E2E。T18～T28 已逐步补齐编辑器、session/history、恢复、冲突、另存、资源后端、两种 adapter、保存、图片和页面状态测试；T29 新增每窗口原生菜单状态、动作路由、状态栏及启动页菜单重置测试。当前 Rust 全量为 174 项通过、1 项手动性能探针忽略，Vitest 汇总为 168 项。第二阶段不得删除或弱化这些基线来换取绿灯。
...
- 具体完成情况：T25 已将 visual/source 接入生产 P1，T26 已增加真实保存和窗口结算；T27 已用组件测试覆盖冲突/恢复/另存、P2 根入口、外部删除保护和安全关闭门禁。T28 已覆盖图片流程和 1280/820/740 px 弹层；T29 又验证 1280/1050/820/740 px 与 740×460 低高度工作台、文件树抽屉焦点、状态栏收敛和无根滚动。最新 macOS `.app` 已通过真实 Tauri IPC 打开 fixture 文档，并验证启动页/工作台菜单状态及原生菜单模式切换；该人工证据不替代阶段 T31 的隔离桌面 E2E、原生图片输入、系统关闭/退出或 Windows 验收。
...
- 具体完成情况：T29 已在本机最新生产 `.app` 通过真实 Tauri IPC 打开 P2、fixture P1 和 `note.md`，核对原生文件/编辑/显示菜单启用态并用显示菜单切换源码模式；没有执行编辑保存、冲突、恢复或图片的隔离 E2E。T31 仍需补完整桌面用例与远端 macOS/Windows 矩阵；Windows 原生系统对话框、拖放、剪贴板、菜单和辅助技术仍需人工证据，CI 不能替代。
