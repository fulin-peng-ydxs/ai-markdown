...
- 当前 `package.json` 尚未包含 Markdown 编辑器依赖。2026-07-22 通过 npm registry 核实的候选精确版本为 `@milkdown/kit@7.21.3`、`@milkdown/react@7.21.3`、`codemirror@6.0.2`、`@codemirror/lang-markdown@6.5.1`，均声明 MIT；是否最终写入锁文件必须经过 T18 许可证、包体和往返 PoC 门禁。
...
- 第一阶段测试基线为 115 个 Rust 单测、3 个路径工具测试、18 个树状态测试、4 个永久删除反馈测试、35 个 React 测试、1 个 fixture 测试、4 个许可证测试和 4 个桌面 E2E。第二阶段不得删除或弱化这些基线来换取绿灯。
...
| --- | --- | --- | --- | --- | --- |
| R1 | Windows/macOS 本地优先桌面应用 | 在既有桌面壳内增加离线编辑、保存、恢复和另存；不重做安装/授权 | 待开始 | T18、T29～T32 | 双平台类型/构建/E2E；离线编辑保存；Windows 原生 UI 保持人工项 |
| R2 | 文件与目录管理 | 复用读取、watch、安全写并接入当前编辑会话和图片资源文件；不改变文件树 CRUD 语义 | 待开始 | T19、T21、T22、T26、T28、T30～T31 | 临时工作区集成、外部变化、磁盘/UI 一致性 |
| R3 | 所见即所得 Markdown 编辑 | 完整纳入本阶段 | 待开始 | T18、T19、T23、T25、T30～T32 | 语法语料往返、编辑交互、剪贴板、IME、E2E |
...
| R6 | 图片粘贴、拖放与资源管理 | 完整纳入本阶段 | 待开始 | T18、T22、T23、T28、T30～T32 | 粘贴/拖放/选择、重名、相对路径、失败回滚 |
...
| R10 | Markdown 源码模式 | 完整纳入本阶段 | 待开始 | T18、T19、T24、T25、T30～T32 | 源码编辑、查找替换、异常语法和往返测试 |
...
- 无阻塞计划启动的问题。
- 非阻塞：T18 若实测包体、IME、raw 语法或性能未过门禁，必须暂停开发并带证据重新确认编辑器方案；该结果不能在计划生成时预写为通过。
