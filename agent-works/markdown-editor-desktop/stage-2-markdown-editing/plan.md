...
- 第一阶段测试基线为 115 个 Rust 单测、3 个路径工具测试、18 个树状态测试、4 个永久删除反馈测试、35 个 React 测试、1 个 fixture 测试、4 个许可证测试和 4 个桌面 E2E。T18 新增 7 个编辑器 PoC 测试，T19 新增 28 个 session/history/AST 契约测试与 1 个 P1 陈旧读取回归，T20 及其整改新增 21 个恢复仓储/契约测试，T21 新增 16 个冲突/另存/契约测试，T22 新增 15 个资源偏好、导入和 raw IPC 契约测试，T23 新增 22 个 Milkdown adapter/组件/性能与安全策略测试，T24 新增 15 个 CodeMirror adapter/组件/原始换行投影测试，T25 新增生产 AST 兼容性、统一编辑壳、只读投影和 P1 降级回归；当前 Rust 全量为 167 项，`pnpm test` 的 Vitest 汇总为 124 项。第二阶段不得删除或弱化这些基线来换取绿灯。

### 2.2 已有代码与可复用能力

- `src/features/workbench/WorkspaceWorkbench.tsx` 当前已接入文件树、读取、CRUD、watch、窗口决策和真实 `DocumentEditorShell`，并已消费单一 `DocumentSession`、生产 Remark/GFM 兼容性解析、文档级陈旧读取保护及排版/源码 adapter；仍没有保存控制器、恢复/冲突可见流程或图片输入消费者。
...
| R11 | 操作与文件状态反馈 | 完成本阶段编辑、保存、恢复、只读、冲突和图片异步状态 | 进行中 | T19～T32 | T25 已在 P1 提供模式、只读、兼容性降级、编辑器初始化、统一撤销/重做及现有 session 保存状态的非颜色反馈；自动保存结果、恢复、冲突、另存和图片异步消费者仍待 T26～T31 |
...
| R30 | 核心命令键盘操作 | 编辑、撤销/重做、保存/另存、模式切换、查找和弹层焦点子集 | 进行中 | T23～T25、T27～T32 | T25 已让 P1 工具栏消费统一撤销/重做、格式、模式和当前文档查找命令；保存/另存、原生菜单、弹层及双平台快捷键仍待 T26～T32 |
...
| 必须实现 | R2 | 文件/目录管理 | 编辑会话接入读取、watch、安全写和资源文件 | T19、T21、T22、T26、T28、T30～T31 | 进行中 | 部分覆盖 | T19 已接入统一读取会话；T21 已接入冲突覆盖/另存；T22 已接入根授权内资源写入与 watcher 登记；自动保存及后续大纲/搜索联动仍待实现 |
...
| 必须实现 | R5 | 保存/恢复/冲突 | 完整单文档链路和当前窗口门禁 | T19～T21、T25～T27、T29～T32 | 进行中 | 部分覆盖 | T19 已建立保存/安全状态，T20 已实现持久恢复仓储，T21 已实现冲突覆盖与安全另存后端契约；自动保存、可见冲突/恢复流程和窗口门禁仍待实现，阶段 3 扩展全部页签关闭检查 |
...
| 必须实现 | R11 | 真实反馈 | 编辑/保存/恢复/冲突/图片全状态 | T19～T32 | 进行中 | 部分覆盖 | T25 已让 P1 呈现模式、只读、兼容性降级、初始化错误和现有 session 保存状态；自动保存、恢复、冲突、另存与图片异步消费者仍待 T26～T31 |
...
| 必须实现 | R30 | 键盘流程 | 编辑/保存/模式/当前文档查找/弹层焦点 | T23～T25、T27～T32 | 进行中 | 部分覆盖 | T25 已在 P1 统一接入撤销/重做、格式、模式与当前文档查找；保存/另存、原生菜单、弹层及页签/阅读/工作区搜索仍待对应阶段 |
...
- 具体完成情况：T19 已覆盖 session/history；T20 已覆盖恢复仓储；T21 已新增 16 个冲突/另存/契约测试；T22 已新增 15 个偏好、图片资源和 raw IPC 契约测试；T23/T24 已覆盖两种 adapter；T25 已覆盖生产 AST 兼容性、模式切换、统一历史、只读投影、陈旧解析拒绝、即时输入提交、空态、查找转源码和 P1 密集结构降级。自动保存 debounce/single-flight 与后续弹层仍待 T26/T27。
...
- 具体完成情况：T20 已覆盖 recovery CRUD/授权；T21 已覆盖外部修改→冲突覆盖、工作区内外另存、一次性令牌、源/目标 revision 与 Rust↔TS parity；T22 已覆盖偏好仓储、资源导入/确认/取消、唯一命名、失败清理和资源 DTO/tag/enum parity。读取→正式编辑→自动保存、资源到 Markdown 的页面集成和窗口 close/replace intent 仍待后续任务。
...
- 具体完成情况：T25 已将 visual/source 接入生产 P1，并通过组件测试验证格式、统一撤销/重做、空文档、只读、兼容性降级、加载失败和工作台接线；真实 Chromium PoC 复核排版→源码→排版、刚输入即切换不丢内容、700 px 编辑器壳无横向溢出和干净控制台。该 PoC 使用生产组件但不是 Tauri P1 IPC 页面；保存/冲突/恢复/图片、1280/1050/820/740 px 完整 P1 桌面矩阵仍待 T26～T31。
