...
| --- | --- | --- | --- | --- | --- |
| R1 | Windows/macOS 本地优先桌面应用 | 在既有桌面壳内增加离线编辑、保存、恢复和另存；不重做安装/授权 | 进行中 | T18、T29～T32 | T31 已完成本机 macOS 8/8 真 Tauri/WebKit E2E 和生产隔离构建；最新远端 macOS/Windows CI 仍待推送，Windows 原生 UI 保持人工项 |
| R2 | 文件与目录管理 | 复用读取、watch、安全写并接入当前编辑会话和图片资源文件；不改变文件树 CRUD 语义 | 进行中 | T19、T21、T22、T26、T28、T30～T31 | T31 已在临时工作区完成打开、编辑、资源落盘、保存、重开、外部修改与恢复回归；最新 Windows 平台证据仍待远端 CI |
| R3 | 所见即所得 Markdown 编辑 | 完整纳入本阶段 | 进行中 | T18、T19、T23、T25、T30～T32 | T31 已用真实桌面链路验证排版/源码编辑、保存和重开；系统输入法候选窗与最新 Windows 证据仍待 T31 远端/人工项和 T32 验收 |
| R4 | 当前文档大纲 | 阶段 6 实现；本阶段只提供增量内容事件，不渲染大纲 | 跳过 | - | 映射审查确认没有假大纲或占位任务 |
| R5 | 自动保存、恢复与外部冲突 | 完成单文档链路；多页签关闭检查由阶段 3 扩展，搜索索引异常协同由阶段 7 承接 | 进行中 | T19～T21、T25～T27、T29～T32 | T31 已完成外部修改保留本地内容、自动恢复快照和显式载入恢复且不先覆盖源文件的真 IPC E2E；远端双平台矩阵与 T32 总体验收仍待完成 |
| R6 | 图片粘贴、拖放与资源管理 | 完整纳入本阶段 | 进行中 | T18、T22、T23、T28、T30～T32 | T31 已用 WebView `DataTransfer/File` 事件贯通 Rust 原始字节上传、确认、Markdown 插入、保存和重开；原生选择器、系统剪贴板/Finder 拖入与最新 Windows 证据仍是人工/远端未验证项 |
...
| R10 | Markdown 源码模式 | 完整纳入本阶段 | 进行中 | T18、T19、T24、T25、T30～T32 | T31 已在真实 WebKit 会话内切换源码并把编辑结果保存、重开；系统 IME 和最新 Windows WebView2 仍未验证 |
| R11 | 操作与文件状态反馈 | 完成本阶段编辑、保存、恢复、只读、冲突和图片异步状态 | 进行中 | T19～T32 | T31 已覆盖保存、外部修改、恢复与图片成功链的真实桌面状态；系统辅助技术、原生系统输入和远端双平台异常矩阵仍待完成 |
...
| R14 | 一目录一窗口与多窗口生命周期 | 仅将当前单文档保存门禁接入关闭/当前窗口替换；阶段 3 扩展为全部页签 | 进行中 | T26、T29～T32 | T31 已回归单窗口临时工作区打开、保存与重开；系统关闭/应用退出人工链路、最新双平台 CI 和阶段 3 全页签结算仍未完成 |
...
| R31 | 无障碍基础 | 编辑器、保存状态、冲突/恢复/资源弹层子集 | 进行中 | T23～T25、T27～T32 | T31 沿用并回归 P1/P2 焦点与可观察状态，未新增视觉结构；系统辅助技术、IME 候选窗和 Windows 原生证据仍待人工项/T32 登记 |
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| 必须实现 | R1 | 双平台、本地优先 | 既有桌面壳内增加离线编辑/恢复/另存并做双平台回归 | T18、T29～T32 | 进行中 | 部分覆盖 | T31 已取得本机 macOS 真 Tauri/WebKit 8/8 与正式构建隔离证据；最新远端 macOS/Windows 矩阵和 Windows 原生人工项尚未完成 |
| 必须实现 | R2 | 文件/目录管理 | 编辑会话接入读取、watch、安全写和资源文件 | T19、T21、T22、T26、T28、T30～T31 | 进行中 | 部分覆盖 | T31 已在独立临时工作区完成读取、图片落盘、保存、重开、外部修改与恢复跨模块回归；后续大纲/搜索联动与最新 Windows 证据仍待对应任务 |
| 必须实现 | R3 | 所见即所得编辑 | Milkdown、统一模型、格式化、粘贴和往返 | T18、T19、T23、T25、T28、T30～T32 | 进行中 | 部分覆盖 | T31 已在真桌面会话验证排版/源码连续编辑、保存和重开；系统 IME 候选窗、最新 Windows 与 T32 总体验收仍待完成 |
| 必须实现 | R4 | 大纲 | 暂不纳入 | - | 跳过 | 未覆盖 | 阶段 6 |
| 必须实现 | R5 | 保存/恢复/冲突 | 完整单文档链路和当前窗口门禁 | T19～T21、T25～T27、T29～T32 | 进行中 | 部分覆盖 | T31 已用真实 IPC 覆盖保存重开、外部修改保留本地内容、自动恢复快照与显式恢复；远端双平台矩阵、T32 验收和阶段 3 全页签关闭检查仍待完成 |
| 必须实现 | R6 | 图片/资源 | 粘贴、拖放、选择、资源偏好与相对链接 | T18、T22、T23、T28、T30～T32 | 进行中 | 部分覆盖 | T31 已以 WebView 文件拖放事件贯通 Rust 上传/确认、相对 Markdown、保存和重开；系统剪贴板/Finder 原生拖入、原生选择器和最新 Windows 证据仍待人工/远端验证 |
| 必须实现 | R7 | 主题颜色 | 只消费现有 token | - | 跳过 | 未覆盖 | 阶段 4/5 |
| 必须实现 | R8 | 布局调宽 | 只做现有响应式回归 | - | 跳过 | 未覆盖 | 阶段 4，不以 T31 回归冒充实现 |
| 必须实现 | R9 | 专注/分页阅读 | 暂不纳入 | - | 跳过 | 未覆盖 | 阶段 6 |
| 必须实现 | R10 | 源码模式 | CodeMirror、统一内容源、异常语法保留 | T18、T19、T24、T25、T30～T32 | 进行中 | 部分覆盖 | T31 已在真实 WebKit 会话切换源码并保存重开；系统 IME、最新 Windows WebView2 和 T32 总体验收仍待完成 |
| 必须实现 | R11 | 真实反馈 | 编辑/保存/恢复/冲突/图片全状态 | T19～T32 | 进行中 | 部分覆盖 | T31 已取得真实桌面保存、外部修改、恢复和图片成功链证据；系统辅助技术、原生系统输入与远端双平台异常矩阵仍待完成 |
| 必须实现 | R12 | 全文搜索 | 暂不纳入 | - | 跳过 | 未覆盖 | 阶段 7；文档内查找不替代它 |
| 必须实现 | R13 | 多页签 | 暂不纳入 | - | 跳过 | 未覆盖 | 阶段 3 |
| 必须实现 | R14 | 多窗口生命周期 | 当前单文档关闭/根替换门禁 | T26、T29～T32 | 进行中 | 部分覆盖 | T31 已回归临时工作区打开、保存和重开；系统关闭/应用退出人工链路与最新双平台 CI 仍未完成，阶段 3 再扩展全部页签 |
...
| 必须实现 | R31 | 无障碍 | 编辑器与本阶段弹层/状态子集 | T23～T25、T27～T32 | 进行中 | 部分覆盖 | T31 回归既有 P1/P2 焦点和可观察状态但未替代系统辅助技术；IME 候选窗和 Windows 原生证据仍待人工项/T32 登记 |
...
- 具体完成情况：T25 已将 visual/source 接入生产 P1，T26 已增加真实保存和窗口结算；T27 已用组件测试覆盖冲突/恢复/另存、P2 根入口、外部删除保护和安全关闭门禁。T28 已覆盖图片流程和 1280/820/740 px 弹层；T29 又验证 1280/1050/820/740 px 与 740×460 低高度工作台、文件树抽屉焦点、状态栏收敛和无根滚动。T31 已在本机 macOS 用独立临时工作区完成 P1/P2 真 IPC、1100/740 px、两模式编辑、图片、保存、外部修改和恢复回归；该 WebView 自动化不替代原生图片输入、系统关闭/退出、系统辅助技术或 Windows 验收。
...
- 具体完成情况：T20 已覆盖 recovery workspace 身份绑定；T21 已覆盖另存单目标授权；T22 已覆盖资源目录绝对/`..`/非法名/symlink 拒绝、MIME/签名不一致、SVG/超限、错误 workspace、token 重放/过期/并发和取消前目标变化。Web 输入使用 raw bytes + opaque upload id，确认命令不接收绝对路径。T31 已在 macOS 真 Tauri IPC 验证 WebView 文件输入到资源落盘，但原生选择器、系统剪贴板/Finder 拖入和 Windows 平台运行时仍未验证。
...
- 发布前检查：第二阶段仍只生成内部未签名测试构建；T31 本机 8/8 不是双平台结论，必须在最新提交取得远端 macOS/Windows 双绿且 T32 文档/需求复核通过后，才能宣称阶段完成。
