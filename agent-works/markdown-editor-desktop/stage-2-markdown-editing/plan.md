...
- 第一阶段测试基线为 115 个 Rust 单测、3 个路径工具测试、18 个树状态测试、4 个永久删除反馈测试、35 个 React 测试、1 个 fixture 测试、4 个许可证测试和 4 个桌面 E2E。T18 新增 7 个编辑器 PoC 测试，T19 新增 28 个 session/history/AST 契约测试与 1 个 P1 陈旧读取回归，T20 及其整改新增 21 个恢复仓储/契约测试，T21 新增 16 个冲突/另存/契约测试；当前 Rust 全量为 152 项，`pnpm test` 的 Vitest 汇总为 71 项。第二阶段不得删除或弱化这些基线来换取绿灯。
...
| R2 | 文件与目录管理 | 复用读取、watch、安全写并接入当前编辑会话和图片资源文件；不改变文件树 CRUD 语义 | 进行中 | T19、T21、T22、T26、T28、T30～T31 | T19 已让 P1 读取进入统一 session；T21 已让冲突覆盖复用安全写并在成功后登记 watcher，另存工作区内目标也登记自身写入；自动保存消费与资源链路仍待后续任务 |
...
| R10 | Markdown 源码模式 | 完整纳入本阶段 | 进行中 | T18、T19、T24、T25、T30～T32 | T19 已建立 source-only 解析证据、统一内容与跨模式历史；CodeMirror 正式 adapter、真实 IME 与可见模式切换仍待实现 |
| R11 | 操作与文件状态反馈 | 完成本阶段编辑、保存、恢复、只读、冲突和图片异步状态 | 进行中 | T19～T32 | T19 已以 union 表达文档/保存状态，T20/T21 已提供恢复、冲突、令牌过期、目标变化和另存失败稳定错误；可见状态组件和产品交互仍待后续任务 |
...
| 必须实现 | R2 | 文件/目录管理 | 编辑会话接入读取、watch、安全写和资源文件 | T19、T21、T22、T26、T28、T30～T31 | 进行中 | 部分覆盖 | T19 已接入统一读取会话；T21 已让冲突覆盖复用 safe-write，并在冲突覆盖/工作区内另存成功后登记 watcher；自动保存、资源、大纲/搜索联动后续补 |
| 必须实现 | R3 | 所见即所得编辑 | Milkdown、统一模型、格式化、粘贴和往返 | T18、T19、T23、T25、T30～T32 | 进行中 | 部分覆盖 | T19 已完成统一模型、有界历史和 adapter 契约；真实 AST parser、IME 和正式 P1 编辑器尚未完成 |
...
| 必须实现 | R10 | 源码模式 | CodeMirror、统一内容源、异常语法保留 | T18、T19、T24、T25、T30～T32 | 进行中 | 部分覆盖 | T19 已建立单一 Markdown 内容源、source-only AST 证据契约和跨模式历史；CodeMirror 正式 adapter 与可见模式切换仍待 T24/T25 |
| 必须实现 | R11 | 真实反馈 | 编辑/保存/恢复/冲突/图片全状态 | T19～T32 | 进行中 | 部分覆盖 | T19 已建立可枚举文档/保存/内容安全状态，T20/T21 已提供恢复、冲突、令牌和另存的稳定失败契约；可见反馈和完整异步消费者仍待后续任务 |
...
- 具体完成情况：T19 已覆盖 session/history；T20 已覆盖恢复仓储；T21 已新增 16 个冲突/另存/契约测试。自动保存、editor adapter、资源配置及页面状态仍待后续任务。
...
- 具体完成情况：T20 已覆盖 recovery CRUD/授权；T21 已覆盖外部修改→冲突覆盖、工作区内外另存、一次性令牌、源/目标 revision 与 Rust↔TS 顶层字段、tag、enum parity。读取→正式编辑→自动保存、资源导入和窗口 close/replace intent 仍待后续任务。
...
- 具体完成情况：T20 已覆盖 recovery workspace 身份绑定；T21 已覆盖原生目标提案后只凭一次性令牌提交、工作区外不扩根授权、token 重放/过期/并发、父目录失效、目标换靶和 Unix symlink 插入。前端任意绝对路径没有进入确认命令。Windows 目标占用与扩展路径运行时仍待 T31。
...
- 具体完成情况：T21 已覆盖 UTF-8 BOM、LF/CRLF/CR/mixed、非法 UTF-8、只读、二次外部变化、写入故障、取消和同名目标覆盖；编辑器 IME、大文档挂载、进程退出、磁盘满、图片及 Windows 目标占用仍待后续任务。
