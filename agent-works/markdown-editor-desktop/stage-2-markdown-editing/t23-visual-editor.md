...
- `visualEditorPolicy.ts` 实现无镜像字节缓冲的 UTF-8 计数和块复杂度门槛：当前 `≤2 MiB 且 ≤2000 块` 才进入排版模式；更大或更碎的文档在创建 Milkdown 前返回源码降级。
...
- 约 100 KiB、5680 个短段落的极端语料：策略校准前挂载约 813 ms，单次更新约 7.8 秒。该结果证明字节门槛不足，因此新增 2000 块复杂度门槛。
...
- `pnpm typecheck`。
- `pnpm test`：12 个 Vitest 文件、91/91 通过；其中 T23 新增 20 个 adapter/组件/策略测试，新增覆盖剪贴板拒绝、资源请求异常以及已挂载投影跨越性能门槛时移除陈旧正文。
