...
> 验收后补强：用户于 2026-07-24 确认目录图片移动风险策略，T33 已在不改变需求编号的前提下补上“资源目录或含图片目录移动前提示未打开文档可能断链”的确认链路。T32 以下 8/8 和远端双绿数字仍是当时验收证据；当前 HEAD 的 9/9 已由 run `30082725332` 在 macOS/Windows 复验，详见 `t33-directory-image-move-warning.md`。
>
> 页面审查补强：T34 已把保存状态收敛为状态栏单一位置，恢复当前文档路径，并让工具栏固定保存/另存/查找、只滚动模式与格式区；当前 9/9 已在 macOS/Windows runner 验证上述编辑 chrome，详见 `t34-editor-chrome-ux-refinement.md`。
...
- T32 验收当时的远端双平台证据以已推送的 `71cab73070bce27ec59fc8e35c670680d6ee2021` / run `30063241410` 为准。
- 后续 T33/T34 与文档收口已随当前 HEAD 推送；最新阶段证据改由 run `30082725332` 承担，不用旧 run 替代新提交证据。
...
- 验收后当前 HEAD `914ad8413b30569ab1c704dc1a55f15d3ed78c59` 对应 run `30082725332`，macOS/Windows 均通过当前非桌面门禁、9/9 桌面 E2E、未签名生产构建与 artifact 上传；产物为 `plainroot-macos-30082725332` 和 `plainroot-windows-30082725332`。该 run 覆盖 T33/T34，不改写前两行的 T32 历史数字。
- T32 run `30063241410` 的 macOS artifact digest：`7587bca0661f2b3383d1f96f619b056f679609f6cb323045b6432d35c7c801a8`。
- T32 run `30063241410` 的 Windows artifact digest：`79370e5f83bc7254dfa9f88061b2e1aefea88202713502f5dac3a11bf457c665`。
...
- T32 验收当时没有针对其本地提交的独立远端结果；当前阶段 HEAD 已由 run `30082725332` 完成远端复验。
