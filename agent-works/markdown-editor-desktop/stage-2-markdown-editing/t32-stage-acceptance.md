...
| R6 | PNG/JPEG/GIF/WebP 受控导入、目录偏好、相对链接、缺失重定位和移动调整已落地 | 系统剪贴板、Finder/Explorer 原生拖入和多图峰值内存仍需人工/性能证据 |
...
- 图片链仍保留已登记的非阻塞微边界：JPEG/WebP 采用保守签名信封，带尾随数据的合法文件可能被拒绝；引用式图片定义不在文档移动后的自动链接改写范围；多图 Blob 读取尚无全局并发和总量预算。具体实现证据与验证边界见 `t28-image-assets-ui.md`、`t31-e2e-cross-platform-ci.md` 及 `../architecture/markdown-document-editing.md` 第 9 节，本次验收没有将它们改写为已解决。
