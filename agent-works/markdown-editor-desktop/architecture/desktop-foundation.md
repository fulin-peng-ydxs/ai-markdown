...
| 排版编辑 adapter | `src/features/editor/adapters/milkdown/` | 将 Milkdown CommonMark/GFM、选择/锚点、格式与结构命令、三态剪贴板载荷、受控图片请求和性能门槛映射为统一 `EditorAdapter`；支持只读与异步销毁 | 不持有文件保存、跨模式内容或第二套权威撤销历史；超过 2 MiB 或估算 2000 块时前置返回源码降级；T25 才接入 P1 |
...
- 排版可交互性同时受 UTF-8 字节数和块节点密度约束；当前证据门槛为 `≤2 MiB 且 ≤2000 块`，超出时在创建 Milkdown 前转源码模式。该门槛是可复测的技术安全值，不是 Markdown 文件总上限。
