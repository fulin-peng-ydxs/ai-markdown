...
当前架构覆盖 P2 工作区启动页、P1 编辑工作台、本地文件与窗口底座，并概览每页签 `DocumentSession` runtime、可见页签交互、保存/恢复、资源、菜单和内容无关页签会话仓储与底座之间的边界。编辑子系统的内部状态机和数据流由专项文档维护。P1 已消费多 session runtime、可见页签栏、最近关闭、当前会话元数据写入、全页签两阶段结算、文件操作成功后的多 runtime 批量重映射/关闭，以及窗口替换、关闭和退出 intent；P1/P2 已共用三值打开偏好。已有会话启动恢复、原生页签命令、大纲、全文搜索、主题工作室和分页阅读仍未落地。
...
| P1 编辑工作台 | `src/features/workbench/`、`src/features/tabs/` | 渐进文件树、按 Rust 路径身份唯一打开/聚焦文档、每文档独立 session/history/save controller、单一活动排版/源码 editor、可见 tablist、排序/溢出/最近关闭、当前/其他/右侧/全部的两阶段安全关闭、窗口替换/关闭/退出的同批结算、当前会话元数据写入、文件操作前影响分析与磁盘成功后的多 runtime 路径/移除提交、格式栏、持续状态栏、恢复/冲突/另存、受控图片、文件 CRUD、监听、工作区切换和原生菜单状态投影 | 不包含已有会话启动恢复，也不包含大纲、工作区搜索或虚假磁盘成功状态 |
