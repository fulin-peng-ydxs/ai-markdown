...
当前架构覆盖 P2 工作区启动页、P1 编辑工作台、本地文件与窗口底座，并概览每页签 `DocumentSession` runtime、可见页签交互、保存/恢复、资源、菜单和内容无关页签会话仓储与底座之间的边界。编辑子系统的内部状态机和数据流由专项文档维护。P1 已消费多 session runtime、可见页签栏、最近关闭、当前会话元数据写入、全页签两阶段结算，以及文件操作成功后的多 runtime 批量重映射/关闭；已有会话启动恢复、窗口替换/退出接线、大纲、全文搜索、主题工作室和分页阅读仍未落地。
...
| P1 编辑工作台 | `src/features/workbench/`、`src/features/tabs/` | 渐进文件树、按 Rust 路径身份唯一打开/聚焦文档、每文档独立 session/history/save controller、单一活动排版/源码 editor、可见 tablist、排序/溢出/最近关闭、当前/其他/右侧/全部的两阶段安全关闭、当前会话元数据写入、文件操作前影响分析与磁盘成功后的多 runtime 路径/移除提交、格式栏、持续状态栏、恢复/冲突/另存、受控图片、文件 CRUD、监听、工作区切换和原生菜单状态投影 | 不包含已有会话启动恢复、窗口替换/退出消费批量弹层，也不包含大纲、工作区搜索或虚假磁盘成功状态 |
...
| 窗口与菜单 | `src-tauri/src/window.rs`、`src-tauri/src/menu.rs` | 一目录一窗口、当前/新窗口决策、根会话协调、单实例转交、原生菜单、按窗口保存编辑菜单状态，以及系统关闭/菜单关闭/当前窗口根替换/应用退出的非阻塞结算意图 | 前端已具备混合阻塞批次与一次性页签提交，但窗口 intent 尚未在 T41 消费该可见决策；coordinator mutex 不跨越前端等待；只有聚焦窗口活动 session 可消费的菜单项启用 |
...
- `pnpm test:e2e` 使用独立 identifier、临时状态目录和每套件临时复制工作区，当前本地 10 条用例覆盖 P2/P1 真实 Tauri IPC、P2 1100/740 px 布局与焦点、P1 1100/1050/820/740 px 编辑 chrome、三文档 runtime 的单 adapter/RSS 门禁、两模式编辑、WebView 文件输入到 Rust 图片上传、保存重开、外部修改内容安全、恢复和目录图片移动风险确认/取消。确定性业务流程重试为 0；只有驱动连接层可保留有界启动重试。
