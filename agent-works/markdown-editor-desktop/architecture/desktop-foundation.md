...
本文描述 Plainroot 当前已经落地的桌面底座模块、数据流、权限边界和运行约束。产品范围与最终验收以 `../requirement.md` 为准，各阶段任务状态与验证证据以对应阶段目录的 `plan.md` 为准；Markdown 统一会话、两种编辑投影、保存/恢复/冲突与图片资源的详细架构以 `markdown-document-editing.md` 为专项事实源；页签、窗口会话、全页签结算和平台页签命令以 `tab-window-lifecycle.md` 为专项事实源；代码、清单、配置和自动化测试是实现事实源。

当前架构覆盖 P2 工作区启动页、P1 编辑工作台、本地文件与窗口底座，并概览每页签 `DocumentSession` runtime、可见页签交互、保存/恢复、资源、菜单和内容无关页签会话仓储与底座之间的边界。编辑子系统的内部状态机和数据流由专项文档维护。P1 已消费多 session runtime、可见页签栏、最近关闭、当前会话元数据写入、全页签两阶段结算、文件操作成功后的多 runtime 批量重映射/关闭、窗口替换/关闭/退出 intent、既有会话的轻量页签恢复和惰性加载，以及聚焦窗口原生页签命令；P1/P2 已共用三值打开偏好。大纲、全文搜索、主题工作室和分页阅读仍未落地。
...
| P2 工作区启动页 | `src/features/launcher/` | 文件夹/Markdown 选择、授权范围确认、最近记录、失效重授权、根窗口恢复、恢复快照入口、打开位置决策和全局打开偏好设置 | 不在启动页直接写恢复正文；先打开并授权对应工作区，由 P1 消费快照；不删除本地目录 |
| P1 编辑工作台 | `src/features/workbench/`、`src/features/tabs/` | 渐进文件树、按 Rust 路径身份唯一打开/聚焦文档、每文档独立 session/history/save controller、单一活动排版/源码 editor、可见 tablist、排序/溢出/最近关闭、当前/其他/右侧/全部的两阶段安全关闭、窗口替换/关闭/退出的同批结算、当前会话元数据写入、已有会话轻量恢复/活动项回退/惰性加载/失败隔离、文件操作前影响分析与磁盘成功后的多 runtime 路径/移除提交、格式栏、持续状态栏、恢复/冲突/另存、受控图片、文件 CRUD、监听、工作区切换和按聚焦窗口状态驱动的原生编辑/页签菜单 | 不包含大纲、工作区搜索、阅读、主题或虚假磁盘成功状态 |
| 共享前端组件 | `src/components/`、`src/features/workspace-open/` | `AppDialog`、`AsyncStatePanel` 与 `focusContainment` 统一对话框、状态优先级和焦点生命周期；P1/P2 共用打开位置决策和三值偏好弹层 | 页面业务状态保持在各自 feature；偏好只决定打开位置，不绕过授权、同目录聚焦或页签结算；仅在第二个同职责消费者出现后抽取 |
...
| 契约与非桌面回归门禁 | `src-tauri/src/contract_test.rs`、`src/features/editor/roundtripCorpus.test.ts`、`package.json` | 对每个 TypeScript 导出 interface、字符串枚举/标签登记 Rust parity 断言；tagged union 按真实 `kind` 逐变体比较字段，字段放错变体也会失败；用生产 Milkdown/CodeMirror adapter 验证 CommonMark/GFM、HTML 和 source-only 语料；`test:stage-3:contracts` 提供阶段专项，`verify:non-desktop` 一次执行 Node、Vitest、两种 Rust feature 口径、fmt、Clippy、类型、生产构建和许可证门禁 | 不替代 Tauri IPC、系统输入法、原生选择器或双平台桌面 E2E；测试文件系统只使用隔离临时目录 |
...
| 版本化窗口页签会话 | `src-tauri/src/window_session.rs`、`commands/window_session.rs`、`src/services/desktop/windowSession.ts`、`src/features/tabs/tabSessionPersistence.ts` | 以独立 manifest/session 保存工作区相对路径、页签顺序、活动路径、模式、选择/锚点和最近关闭；前端对当前会话防抖投影，启动发现既有会话先冻结、P1 消费恢复投影后显式接管写入；Rust 提供 CAS、进程内原子回滚、崩溃中间态前滚、损坏隔离、未知版本保护、非敏感 launcher 摘要和平台路径身份 | 不保存 Markdown、history、恢复正文、绝对路径、窗口像素或布局；完整读取/写入必须匹配当前窗口绑定与授权工作区，P2 只获得页签数量、revision 与问题摘要，不获得文件名或正文 |
...
| 偏好、图片导入与目录移动风险 | `src-tauri/src/preferences.rs`、`src-tauri/src/editor/assets.rs`、`src-tauri/src/fs/mutate.rs`、`commands/editor.rs`、`commands/files.rs`、`commands/workspace.rs`、`src/services/desktop/assets.ts`、`files.ts`、`workspace.ts`、`src/features/editor/assets/`、`src/features/workspace-open/` | 以同一版本化仓储保存全局工作区打开方式和按工作区资源目录；P1/P2/menu 消费三值打开偏好；资源目录继续在授权根内重校验；图片链路通过 raw IPC 或 Rust 原生选择器导入 PNG/JPEG/GIF/WebP，校验签名和 20 MiB 上限后原子写入不覆盖的唯一资源名；目录移动前以有界检查提示可能影响未打开文档 | 偏好不包含正文；打开偏好缺失/读取失败安全回到每次询问且不能跳过结算；SVG 拒绝；原始图片不删除；上传/导入令牌仅进程内、最多 32 项、5 分钟且单次消费；目录风险检查不解析 Markdown、不声称引用命中、不批量改写其他文档 |
...
- 移动打开文档或其包含目录时，先安全结算全部受影响页签并完成磁盘移动，再以 Markdown AST 位置重写所有受影响已加载 `DocumentSession` 中的内联图片 URL；重写结果进入各自 dirty/自动保存/恢复链，磁盘移动失败前不得改写正文。若链接写回失败，目录移动不被虚假回滚，相关页签保持 dirty 并显示真实反馈。移动已配置资源目录或递归包含受支持图片的目录前，Rust 在授权根内执行最多 10,000 项的有界检查，P1 明确提示未打开文档的相对图片链接可能失效；用户取消不移动。该检查不构成跨文档引用索引，未打开文档不会被自动批量重写。
...
- 根工作区提交前由窗口页签会话仓储创建 opaque `windowStateRef`，`plainroot-state-v1.json` 只保存该引用。页签 session 通过 revision/CAS 防止晚到元数据覆盖新顺序；进程内 manifest 提交失败回滚 session，硬终止后初始化只接受同 workspace/ref 且领先一个 revision 的有效 session 并前滚 manifest，其他不一致仍按损坏隔离。
- 完整页签路径读取与新路径身份解析必须同时校验当前 WebView 窗口绑定、工作区授权和 Rust 根内路径。对外身份是 workspace 绑定的 SHA-256 opaque 值，不暴露 canonical root；恢复解析后按该 identity 二次去重，Windows 或 macOS 大小写别名不能形成两个运行时页签。
- manifest 损坏时从校验通过的 v1 session 重建，单 session 损坏仅备份并隔离该项；未知版本原文件不覆盖。孤儿清理只处理 Plainroot 命名、普通文件且明确属于 v1 的记录，不跟随 symlink 或猜测删除未来版本。
...
| 窗口页签会话 | `appDataDir()/plainroot-window-sessions-v1/` | schema v1；manifest 上限 1 MiB/1000 工作区；单 session 上限 1 MiB；最近关闭最多 50 项；只含相对路径和视图元数据 | 私有原子文件 + CAS；进程内双文件提交失败恢复旧 session，硬终止中间态按单 revision 前滚；manifest 可从有效 v1 session 重建；单项损坏隔离；未知版本保留；移除最近记录可重试且只删元数据，不触碰 Markdown 或恢复快照 |
...
| 工作区打开偏好 | `appDataDir()/plainroot-preferences-v1.json` | schema v1 可选全局 `workspaceOpenDisposition`；值为 `ask/current_window/new_window`，旧文件缺失时默认 `ask` | 与资源偏好共用私有原子仓储；读取失败不阻止打开而回到询问；设置页可显式恢复为每次询问；删除文件不影响 Markdown、窗口会话或资源文件 |
...
- 前端测试覆盖许可证策略、路径代数、文件树 reducer、页签纯状态模型/批量结算/原子重映射、fixture、React 页面/组件状态、统一编辑器壳、Milkdown/CodeMirror adapter、生产 Markdown 往返语料、恢复/冲突/另存交互、资源目录、文档相对图片路径、选择/剪贴板部分失败、缺失占位/重新定位、打开页签路径变更和目录图片移动风险确认，以及保存控制器的尺寸分级、单飞/立即追赶、快照限频、失败与结算；Rust 测试继续覆盖授权、路径、状态/恢复/偏好/窗口页签会话仓储、文件操作、安全写、监听、窗口事务、结算 intent、资源导入、受控图片读取和有界目录风险分类。契约总守卫要求每个 TypeScript 导出 interface 和字符串常量都能追溯到 Rust 字段或枚举 parity 断言。
- 所有文件系统测试必须使用带进程 ID、时间与原子序号的统一临时目录工厂；仅依赖时间戳的夹具会在并行测试中碰撞清理日志或临时文件，禁止新增。
- `pnpm test:e2e` 使用独立 identifier、临时状态目录和复制工作区，以四个隔离桌面进程运行 15 条用例：12 条 P1/P2 主链、平台页签组合键、重启前会话写入和重启后恢复。除既有真实 IPC、文件、编辑、图片、恢复与单 adapter/RSS 门禁外，还覆盖 1100/1050/820/760/740 px、窗口替换拒绝和“进程间删除一项后隔离失败、继续恢复其他页签”。确定性业务流程重试为 0；只有驱动连接层可保留有界启动重试。T45 代码提交与其上纯文档提交均已取得 macOS/Windows 15/15 和成对 artifact。
- GitHub Actions 在 macOS/Windows 运行许可证、类型、前端/Rust、四段 15/15 桌面 E2E 和未签名生产构建。T45 代码提交 `9ba28d1bd077b950059b6b474e7da77cf173ea63` 的 run `30322298422` 与其上纯文档提交 `a0268eb2482face230bb6458f085a68afcfaeaed` 的 run `30323620642` 均已双绿并上传成对 artifact。Windows runner 以编译期固定标题稳定 WDIO/WebView2 renderer，因此不覆盖生产动态文档标题路径；WebView 自动化也不替代 Windows 原生系统 UI 的人工验收。
