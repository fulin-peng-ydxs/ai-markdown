...
- `pnpm test:e2e` 使用独立 identifier、临时状态目录和复制工作区，当前本地 macOS 以四个隔离桌面进程通过 15 条用例：12 条 P1/P2 主链、平台原生页签组合键、重启前会话写入和重启后恢复。除既有真实 IPC、文件、编辑、图片、恢复与单 adapter/RSS 门禁外，还覆盖 1100/1050/820/760/740 px、窗口替换拒绝和“进程间删除一项后隔离失败、继续恢复其他页签”。确定性业务流程重试为 0；只有驱动连接层可保留有界启动重试。Windows 与远端 artifact 尚未取得当前提交证据。
