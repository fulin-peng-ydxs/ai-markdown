...
本文描述 Plainroot 当前已经落地的 Markdown 编辑模块，包括统一内容模型、排版/源码投影、保存与恢复、外部冲突、另存副本、图片资源、窗口结算和菜单状态，以及第三阶段已接入 P1 的每页签独立文档 runtime、可见页签交互、已有会话启动恢复和原生页签命令。产品范围与验收口径以 `../requirement.md` 为准，第二阶段任务状态与证据以 `../stage-2-markdown-editing/plan.md` 和 `../stage-2-markdown-editing/t32-stage-acceptance.md` 为准；页签与窗口生命周期专项架构见 `tab-window-lifecycle.md`，第三阶段验收见 `../stage-3-tab-window-lifecycle/t46-stage-acceptance.md`；代码、清单、配置和自动化测试是实现事实源。

`desktop-foundation.md` 负责工作区授权、文件系统、窗口和应用状态等跨模块底座概览；本文是 Markdown 文档编辑子系统的专项事实源。T35～T46 已建立页签状态、元数据仓储、P1 多 session runtime、可见页签容器、最近关闭、当前会话元数据写入、两阶段安全结算、磁盘成功后的多 runtime 路径重映射/批量关闭、窗口替换/关闭/退出保护、既有会话的轻量恢复/活动项回退/惰性加载，以及按聚焦窗口状态驱动的原生页签命令，并通过第三阶段验收。大纲、工作区搜索、分页阅读、主题工作室和完整布局持久化不属于本文所述的已完成能力。
...
- T38 的 `WorkspaceTabBar` 直接消费 manager 快照，提供真实 tablist、同名父路径、公共主状态、单页签关闭、当前窗口拖动/键盘排序和全部页签溢出；`TabOverflowMenu` 与 `TabContextMenu` 共同消费 `TabMenu` 的方向键、Esc 和焦点返回。T40 已启用关闭其他、关闭右侧和关闭全部，T43 又把关闭/重开/切换/批量关闭/所有页签接入原生菜单；全部动作仍委托同一 manager、共享菜单与结算批次，不复制保存判断或命令总线。
...
- 真实 Tauri/WebKit 门禁使用三个文档、36 次可见页签切换，逐次断言页面只有一个 `.ProseMirror` 或 `.cm-editor`，并以测试 feature 的进程 RSS 采样执行增量 ≤128 MiB 的可失败门禁；另验证溢出菜单 Esc 焦点返回及 1100/1050/820/760/740 px 不产生根级横向溢出。当前 macOS WebKit 未暴露 JS heap，因此只以单 adapter 与 RSS 作为已取得证据；JS heap 与双平台内存证据仍未取得，不能因 T45 macOS 桌面链通过而外推。
...
- 第二阶段提交 `914ad8413b30569ab1c704dc1a55f15d3ed78c59` 的 run `30082725332` 已提供 9/9 双平台桌面基线。第三阶段 T45 代码 run `30322298422` 已在提交 `9ba28d1` 上让 macOS/Windows 同时通过非桌面门禁、四段 15/15 桌面链、生产构建和成对 artifact；收口文档 run `30323620642` 又在纯文档提交 `a0268eb` 上重复双绿。macOS 页签切换由原生菜单路由，Windows `Ctrl+PageDown/PageUp` 由聚焦 WebView 平台适配路由，均委托同一 manager。
...
- T45 已在 macOS/Windows 真实验证跨进程恢复、窗口替换拒绝和平台页签组合键，并取得双平台生产 artifact；T46 已完成第三阶段整体复核与验收。真实多窗口整组退出、Windows 原生系统 UI 人工项、R14 的窗口位置/尺寸/三栏布局、R30 的搜索/阅读命令和 R31 的完整无障碍矩阵仍未完成。Windows E2E 固定原生标题，故生产动态文档标题路径仍由 macOS 主链与前端契约覆盖。大纲、工作区搜索、分页阅读、主题预设和完整响应式布局仍属于后续阶段。
