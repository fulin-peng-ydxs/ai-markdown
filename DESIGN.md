...
| `WorkspaceTabBar` | `src/features/tabs/WorkspaceTabBar.tsx` | P1、原生“所有页签”命令 | 消费 manager 快照投影真实 tablist、活动项、同名路径提示和非颜色状态；提供 roving tabindex、鼠标/键盘激活、单项/其他/右侧/全部关闭入口、当前窗口排序与最近关闭入口，页签 viewport 独立横向滚动；原生“所有页签”命令只请求打开既有溢出菜单，具体内容安全决策委托统一结算弹层 |
| `TabMenu` | `src/features/tabs/TabMenu.tsx` | `TabOverflowMenu`、`TabContextMenu`、原生“所有页签”命令的可见消费者 | 共享 menu/分区语义、可聚焦项筛选、方向键/首尾遍历、Esc/Tab/外部点击关闭和焦点返回；溢出定位、最近关闭与页签动作由薄消费者分别配置，原生命令只复用现有消费者和焦点入口，不在菜单内复制页签状态 |
...
- HTML 原型主要表现 macOS 窗口外观；T17 提交已取得 Windows CI 证据。页签切换按平台适配：macOS 由原生菜单消费 `Cmd+Option+Right/Left`，Windows 聚焦工作台消费 `Ctrl+PageDown/PageUp` 并委托同一页签命令处理器；Windows 原生标题栏、菜单、回收站、窗口行为和辅助技术仍需人工验收补证。
