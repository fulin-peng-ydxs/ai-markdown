...
- 当前仓库已完成 T1～T14 的工程、领域/路径、原生窗口、状态、授权、文件读写管理、监听、安全写入、窗口协调、P2 启动页、P1 第一阶段工作台壳及共享对话框/状态契约；当前具备真实系统选择器、最近记录、根目录决策、渐进文件树、只读 Markdown、文件 CRUD/删除/定位入口、窄窗目录抽屉和多窗口打开，但尚无自动保存、恢复快照、完整外部冲突状态机、编辑器/页签/大纲，也没有 Windows 实机、稳定桌面 E2E 与 CI，不得据此表述为 R1、R2、R5、R14 或第一阶段整体完成。
...
- 当前自动化测试覆盖许可证拒绝策略、T2～T11 底座、Rust↔TypeScript 契约 parity、T12 launcher、T13 workbench，以及 T14 对话框焦点圈定、类型化状态优先级、aria-live/role 与非颜色标签，共 111 个 Rust 单元测试、18 个前端树状态测试、4 个永久删除反馈测试和 32 个 React UI/状态测试；双进程 single-instance、系统废纸篓/Finder/Explorer、Windows 分支、稳定桌面 E2E 与 CI 验证仍由后续任务建立。当前许可证扫描为 112 个 Node 包、487 个 Rust 包、0 个阻断项。新增长期启动命令或测试后，必须同时验证启动、就绪、停止和失败方式，再更新本节。
