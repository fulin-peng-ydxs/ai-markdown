...
- 第二阶段当前分支为 `codex/plainroot-stage-2`，执行计划位于 `agent-works/markdown-editor-desktop/stage-2-markdown-editing/plan.md`；T18～T30 已完成，T31 本机实现和 macOS 桌面基线已完成、待远端双平台 CI。当前已建立单一 `DocumentSession`、两种编辑 adapter、自动/手动保存与窗口结算、恢复/冲突/另存、受控图片链路，以及由聚焦窗口真实 session 驱动的原生保存/历史/查找/模式菜单和持续文档状态栏。非桌面门禁覆盖生产 Markdown 往返语料、编辑器状态、Rust 服务与全部 Rust↔TypeScript interface/枚举登记；8 条隔离桌面 E2E 已覆盖两模式编辑、图片、保存、外部修改和恢复。系统 IME、峰值内存、原生系统输入与最新 Windows 产品证据仍未验证。
...
- `tests/`：已建立脱敏工作区 fixture、Node 临时目录工厂和隔离的 P1/P2 Tauri E2E；桌面套件在每次运行时复制独立临时工作区，业务流程不靠重试掩盖确定性失败。`.github/workflows/ci.yml` 已跑通第一阶段双平台门禁；扩展后的 8 条套件当前在 macOS 本机通过，尚无最新远端 Windows 证据。自动化证据不替代 Windows 原生系统交互的人工验收。
...
- 2026-07-24 已在 macOS arm64 实际验证第二阶段当前本地基线：176 个 Rust 测试通过（另 1 项手动性能探针忽略）、Rust fmt/全 feature Clippy、173 个 Vitest、30 个 Node 独立回归、前端生产构建、许可证 727/508/0、Tauri release 构建和真实 Tauri/WebKit 8/8 桌面 E2E。默认全量 `cargo test` 的既有并发夹具碰撞已修复；恢复 revision hash、活动快照注册和 adapter 连续本地事务均有跨层回归守卫。系统 IME、峰值内存、原生系统输入和最新远端 Windows 证据仍未验证。
...
- 当前自动化测试在既有底座、编辑器、恢复/冲突/资源与菜单链路上共 176 个 Rust 测试通过（另 1 项手动性能探针忽略）、3 个工作区路径测试、18 个前端树状态测试、4 个永久删除反馈测试、173 个 React UI/状态/编辑器测试、1 个 fixture 测试、4 个许可证策略测试和 8 个桌面 E2E。`pnpm test` 统一执行 Node、Vitest 与无桌面 feature 的 Rust 服务门禁；`test:editor`、`test:roundtrip`、`test:rust` 可单独定位。提交 `9a1690a` 的 Windows runner 证据只覆盖第一阶段；第二阶段最新改动尚无远端 Windows 证据。许可证扫描为 727 个 Node 包、508 个 Rust 包、0 个阻断项。
