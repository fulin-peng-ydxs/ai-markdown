...
- 第一阶段 T1～T17 已建立并验证系统选择器、最近记录、根目录决策、渐进文件树、文件 CRUD/删除/定位、窄窗目录抽屉、多窗口协调和隔离 P1/P2 桌面测试等底座。第二阶段 T18～T34 又建立单一 `DocumentSession`、两种编辑 adapter、自动/手动保存与窗口结算、恢复/冲突/另存、受控图片链路、目录移动风险提示、原生编辑菜单、固定主操作工具栏和单一保存/路径状态栏。第三阶段分支 `codex/plainroot-stage-3` 的 T35～T44 已建立纯页签集合模型、公共状态投影、内容无关窗口页签会话仓储、真实根会话引用、Rust 平台路径身份、P1 每页签独立 session/history/save controller、活动 editor 单挂载、可见页签交互、最近关闭/当前会话元数据、全页签两阶段结算、磁盘成功后的多 runtime 路径重映射/批量关闭、窗口替换/关闭/退出的全页签保护、可回退打开偏好、既有会话恢复、复用既有 manager/menu/settlement 的原生页签命令与聚焦窗口启用态，以及逐变体契约 parity 和一键非桌面门禁；T45～T46 仍未实施。当前计划见 `agent-works/markdown-editor-desktop/stage-3-tab-window-lifecycle/plan.md`。
- 第二阶段基线提交 `914ad8413b30569ab1c704dc1a55f15d3ed78c59` 的非桌面门禁和 9/9 隔离桌面 E2E 已在 macOS/Windows runner 通过；第三阶段 T35～T44 当前仅取得 macOS 本地证据，尚未推送或取得远端 CI，当前代码 11/11 桌面 E2E 通过，原生走查另确认页签菜单、点击切换、`Cmd+W` 关闭和 `Cmd+Shift+T` 重开。系统 IME、JS heap、长时峰值内存、`Ctrl+Tab` 真实系统输入、真实双窗口退出/替换、真实重启/多窗口恢复，以及 Windows 选择器、回收站、Explorer、菜单和辅助技术仍无完整证据；当前完成状态不得表述为完整 R13/R14/R30、大纲、工作区搜索、分页阅读、主题工作室或完整产品完成。
...
- 2026-07-27 已在 macOS arm64 实际验证 T44 本地基线：204 个 Rust 测试通过（另 1 项手动性能探针忽略）、267 个 Vitest、30 个 Node 独立回归、类型检查、前端生产构建、Rust fmt、全 feature Clippy/测试和许可证 727/511/0。`test:stage-3:contracts` 以 110/110 Vitest 加完整 Rust 服务测试覆盖页签/工作台/仓储/窗口/菜单；tagged union parity 会按 `kind` 逐变体校验字段，字段放错变体的反向测试可失败。`verify:non-desktop` 是统一非桌面门禁。当前代码 11/11 macOS Tauri/WebKit 回归仍为 T43 证据，原生走查确认菜单点击切换、`Cmd+W` 关闭和 `Cmd+Shift+T` 重开；`Ctrl+Tab` 真实输入和 Windows 未验证。T35 的轻量 benchmark 仍只作为本机报告。
...
- 当前自动化测试在既有底座、编辑器、恢复/冲突/资源、菜单、页签模型/runtime、窗口页签仓储和打开偏好链路上共 204 个 Rust 测试通过（另 1 项手动性能探针忽略）、3 个工作区路径测试、18 个前端树状态测试、4 个永久删除反馈测试、267 个 React UI/状态/编辑器/页签测试、1 个 fixture 测试、4 个许可证策略测试和 11 个本地 macOS 桌面 E2E。`pnpm test` 统一执行 Node、Vitest 与无桌面 feature 的 Rust 服务门禁；`test:stage-3:contracts` 组合阶段专项与完整 Rust 服务测试，`verify:non-desktop` 统一执行全部非桌面提交门禁；`test:tabs:performance` 仅输出本机 benchmark。GitHub Actions run `30082725332` 已在 macOS/Windows 完整通过第二阶段的 9 条桌面门禁、生产构建和 artifact 上传，尚未覆盖 T35～T44 本地提交。许可证扫描最近一次为 727 个 Node 包、511 个 Rust 包、0 个阻断项。Windows 原生系统 UI 仍未人工验证。
...
- 修改代码或文档前检查最近一次远端同步时间；超过 2 小时先同步并解决冲突。最近一次远端同步：2026-07-27 16:57 CST（本地 `codex/plainroot-stage-3` 已 fetch 并核对最新 `origin/codex/plainroot-stage-2`，远端尚无第三阶段分支，当前第三阶段提交尚未推送或取得远端 CI 证据）。
