...
- 第二阶段当前分支为 `codex/plainroot-stage-2`，执行计划位于 `agent-works/markdown-editor-desktop/stage-2-markdown-editing/plan.md`；T18～T23 已完成，T24 待开始。当前已建立单一 `DocumentSession`、有界可逆 patch 历史、Milkdown CommonMark/GFM 排版 adapter、独立 recovery/资源偏好仓储，以及冲突覆盖、原生单目标安全另存和受控图片导入后端契约。视觉 adapter 不持有第二份内容/保存/权威历史，撤销重做桥接 session；超过 2 MiB 或 2000 个非空内容行时在挂载前明确转源码模式，紧凑列表/表格也受该门槛约束。P1 仍未消费可编辑 adapter，也没有 CodeMirror 产品 adapter、自动保存、恢复/冲突/资源弹层或完整图片输入消费者；系统 IME、峰值内存和 WebKit/Windows 产品证据仍由 T25/T30/T31 验证。

## 当前与目标代码边界

- `agent-works/markdown-editor-desktop/`：本产品的需求、计划、原型和后续开发留痕；同一产品能力继续复用该目录或其语义明确的阶段子目录。
- `src/`、`src-tauri/`：已建立 P1/P2、共享 `AppDialog`/类型化 `AsyncStatePanel`/`focusContainment`、`WorkspaceTree`、工作区相对路径工具 `workspacePath`、单文档 `DocumentSession`/有界 patch history、Milkdown `VisualMarkdownEditor`/adapter、独立 recovery repository、冲突覆盖/安全另存服务、完整浅色运行时 token、桌面契约/调用封装、渐进文件树/变更/监听状态、永久删除对话框、窗口协调、版本化状态仓储，以及工作区选择、扫描、读取、CRUD、删除、定位、监听、安全写入、恢复快照、一次性冲突覆盖和原生单目标另存命令；公共焦点工具由对话框和窄窗抽屉共同消费，路径工具由树 reducer 和 P1 页面共同消费，P1 只读内容消费统一 session。当前 P1 中央区仍仅真实只读查看，不含自动保存触发/恢复或冲突弹层、正式可编辑消费、页签、大纲或搜索。
...
- 当前自动化测试覆盖许可证策略、T2～T11 底座、Rust↔TypeScript 契约 parity、T12 launcher、T13 workbench、T14 公共组件、T17 焦点/快捷键契约、T18 编辑器 PoC、T19 session/history/AST 契约与 P1 陈旧读取、T20 recovery 原子/容量/损坏/活动保护、T21 冲突覆盖/单目标另存安全边界、T22 资源偏好/图片签名/唯一写入/确认清理/raw IPC、T23 Milkdown adapter/剪贴板/生命周期/性能降级、根启动错误去重、工作区路径代数、临时 fixture、失效清理日志满额回归、平台原子替换和 P1/P2 桌面 E2E，共 167 个 Rust 单元测试、3 个工作区路径测试、18 个前端树状态测试、4 个永久删除反馈测试、93 个 React UI/状态/编辑器测试、1 个 fixture 测试、4 个许可证策略测试和 4 个桌面 E2E；提交 `9a1690a` 的 Windows runner 已通过 105 个平台适用 Rust 用例、4/4 桌面 E2E 和未签名生产构建。T18～T23 改动尚无远端 Windows 证据，且 Windows 原生保存/图片选择对话框、目标占用和系统 UI 仍未人工验证。许可证扫描覆盖默认与 `e2e` Cargo feature，为 727 个 Node 包、508 个 Rust 包、0 个阻断项。新增长期启动命令或测试后，必须同时验证启动、就绪、停止和失败方式，再更新本节。
