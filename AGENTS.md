...
- 当前仓库已完成 T1～T15，T16 已建立 macOS/Windows CI、平台原子替换回归、P1 真 IPC E2E 和测试工具许可证门禁；第二轮远端矩阵的 macOS 已通过，Windows 稳定 API 编译问题已整改待复跑。当前具备真实系统选择器、最近记录、根目录决策、渐进文件树、只读 Markdown、文件 CRUD/删除/定位入口、窄窗目录抽屉、多窗口打开和可重复隔离的 P1/P2 桌面测试，但尚无自动保存、恢复快照、完整外部冲突状态机、编辑器/页签/大纲，也没有 Windows runner 全量测试/E2E 通过结果，不得据此表述为 R1、R2、R5、R14、T16 或第一阶段整体完成。
...
- `tests/`：已建立脱敏工作区 fixture、Node 临时目录工厂和隔离的 P1/P2 Tauri E2E；`.github/workflows/ci.yml` 已实现 T16 双平台门禁。macOS 远端已完整通过；Windows 在 Rust lint 失败后的整改尚待复跑，不得表述为双平台 CI 已通过。
