...
- 第二阶段分支为 `codex/plainroot-stage-2`，执行计划位于 `agent-works/markdown-editor-desktop/stage-2-markdown-editing/plan.md`；T18～T34 已完成本地阶段验收及目录图片移动风险、编辑工作台信息层级补强。当前已建立单一 `DocumentSession`、两种编辑 adapter、自动/手动保存与窗口结算、恢复/冲突/另存、受控图片链路、目录图片移动前的诚实风险提示，以及由聚焦窗口真实 session 驱动的原生保存/历史/查找/模式菜单、固定主操作工具栏和单一持续保存/路径状态栏。非桌面门禁覆盖生产 Markdown 往返语料、编辑器状态、Rust 服务与全部 Rust↔TypeScript interface/枚举登记；已推送基线的 8 条隔离桌面 E2E 在 macOS/Windows runner 通过，本地最新第 9 条又覆盖目录风险确认、取消零副作用及四档编辑 chrome 布局。系统 IME、峰值内存和原生系统输入仍未完成人工验证；阶段完成不得表述为多页签、大纲、工作区搜索、阅读或完整产品完成。

## 当前与目标代码边界

- `agent-works/markdown-editor-desktop/`：本产品的需求、计划、原型和后续开发留痕；同一产品能力继续复用该目录或其语义明确的阶段子目录。
- `src/`、`src-tauri/`：已建立 P1/P2、共享状态/弹层/焦点组件、工作区树与路径工具、单文档 `DocumentSession`、两种编辑 adapter、保存控制器、恢复/冲突/另存组件、图片资源链、目录移动风险检查、固定主操作工具栏、单一保存/路径状态栏和每窗口原生编辑菜单状态。当前 P1 已真实消费排版/源码编辑、自动/手动保存、恢复/冲突/另存、外部删除保护、图片资源、目录风险确认、菜单与状态栏；尚无页签、大纲或工作区搜索。
- `tests/`：已建立脱敏工作区 fixture、Node 临时目录工厂和隔离的 P1/P2 Tauri E2E；桌面套件在每次运行时复制独立临时工作区，业务流程不靠重试掩盖确定性失败。`.github/workflows/ci.yml` 已在提交 `71cab73070bce27ec59fc8e35c670680d6ee2021` 上跑通 macOS/Windows 扩展后的 8 条套件、非桌面门禁、未签名生产构建和 artifact 上传。自动化证据不替代 Windows 原生系统交互的人工验收。
...
- 2026-07-24 已在 macOS arm64 实际验证第二阶段当前本地基线：180 个 Rust 测试通过（另 1 项手动性能探针忽略）、Rust fmt/全 feature Clippy、176 个 Vitest、30 个 Node 独立回归、前端生产构建、许可证 727/508/0、Tauri release 构建和真实 Tauri/WebKit 9/9 桌面 E2E。默认全量 `cargo test` 的既有并发夹具碰撞已修复；恢复 revision hash、活动快照注册、adapter 连续本地事务和目录图片移动风险确认均有跨层回归守卫。系统 IME、峰值内存和原生系统输入仍未完成人工验证。
- `pnpm test:e2e` 每次使用独立临时状态目录与临时复制工作区，真实 macOS Tauri/WebKit 9/9 通过：P2 IPC、1100/740px 布局、入口焦点、P1 工作区打开、排版/源码编辑、WebView 文件输入到 Rust 图片上传、保存重开、外部修改内容安全、恢复仓储、显式恢复和含图片目录移动前的风险确认/取消。默认生产前端产物和 release 二进制确认不含 WDIO、fixture 或 E2E 命令。T17 的生产 `.app` 原生多窗口、Finder、系统废纸篓、监听和第二实例人工证据仍有效；这些 macOS 证据不替代 Windows 原生人工验收。
...
- 当前自动化测试在既有底座、编辑器、恢复/冲突/资源与菜单链路上共 180 个 Rust 测试通过（另 1 项手动性能探针忽略）、3 个工作区路径测试、18 个前端树状态测试、4 个永久删除反馈测试、176 个 React UI/状态/编辑器测试、1 个 fixture 测试、4 个许可证策略测试和 9 个本地桌面 E2E。`pnpm test` 统一执行 Node、Vitest 与无桌面 feature 的 Rust 服务门禁；`test:editor`、`test:roundtrip`、`test:rust` 可单独定位。GitHub Actions run `30063241410` 已在 macOS/Windows 完整通过 T31/T32 已推送基线的 8 条桌面门禁、生产构建和 artifact 上传；T33/T34 第 9 条及编辑 chrome 四档断言尚无新远端证据。许可证扫描为 727 个 Node 包、508 个 Rust 包、0 个阻断项。Windows 原生系统 UI 仍未人工验证。
...
- 修改代码或文档前检查最近一次远端同步时间；超过 2 小时先同步并解决冲突。最近一次远端同步：2026-07-24 15:58 CST（当前分支跟踪 `origin/codex/plainroot-stage-2`；提交 `71cab73070bce27ec59fc8e35c670680d6ee2021` 已推送并通过双平台 CI）。
