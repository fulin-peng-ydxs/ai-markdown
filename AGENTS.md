...
- 第二阶段分支为 `codex/plainroot-stage-2`，执行计划位于 `agent-works/markdown-editor-desktop/stage-2-markdown-editing/plan.md`；T18～T33 已完成本地阶段验收及目录图片移动风险补强。当前已建立单一 `DocumentSession`、两种编辑 adapter、自动/手动保存与窗口结算、恢复/冲突/另存、受控图片链路、目录图片移动前的诚实风险提示，以及由聚焦窗口真实 session 驱动的原生保存/历史/查找/模式菜单和持续文档状态栏。非桌面门禁覆盖生产 Markdown 往返语料、编辑器状态、Rust 服务与全部 Rust↔TypeScript interface/枚举登记；已推送基线的 8 条隔离桌面 E2E 在 macOS/Windows runner 通过，本地最新第 9 条又覆盖目录风险确认与取消零副作用。系统 IME、峰值内存和原生系统输入仍未完成人工验证；阶段完成不得表述为多页签、大纲、工作区搜索、阅读或完整产品完成。

## 当前与目标代码边界

- `agent-works/markdown-editor-desktop/`：本产品的需求、计划、原型和后续开发留痕；同一产品能力继续复用该目录或其语义明确的阶段子目录。
- `src/`、`src-tauri/`：已建立 P1/P2、共享状态/弹层/焦点组件、工作区树与路径工具、单文档 `DocumentSession`、两种编辑 adapter、保存控制器、恢复/冲突/另存组件、图片资源链、目录移动风险检查、持续状态栏和每窗口原生编辑菜单状态。当前 P1 已真实消费排版/源码编辑、自动/手动保存、恢复/冲突/另存、外部删除保护、图片资源、目录风险确认、菜单与状态栏；尚无页签、大纲或工作区搜索。
...
- 当前自动化测试在既有底座、编辑器、恢复/冲突/资源与菜单链路上共 180 个 Rust 测试通过（另 1 项手动性能探针忽略）、3 个工作区路径测试、18 个前端树状态测试、4 个永久删除反馈测试、176 个 React UI/状态/编辑器测试、1 个 fixture 测试、4 个许可证策略测试和 9 个本地桌面 E2E。`pnpm test` 统一执行 Node、Vitest 与无桌面 feature 的 Rust 服务门禁；`test:editor`、`test:roundtrip`、`test:rust` 可单独定位。GitHub Actions run `30063241410` 已在 macOS/Windows 完整通过 T31/T32 已推送基线的 8 条桌面门禁、生产构建和 artifact 上传；T33 第 9 条尚无新远端证据。许可证扫描为 727 个 Node 包、508 个 Rust 包、0 个阻断项。Windows 原生系统 UI 仍未人工验证。
...
- 修改代码或文档前检查最近一次远端同步时间；超过 2 小时先同步并解决冲突。最近一次远端同步：2026-07-24 11:00 CST（当前分支跟踪 `origin/codex/plainroot-stage-2`；提交 `71cab73070bce27ec59fc8e35c670680d6ee2021` 已推送并通过双平台 CI）。
