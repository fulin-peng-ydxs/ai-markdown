...
- 第一阶段 T1～T17 已建立并验证系统选择器、最近记录、根目录决策、渐进文件树、文件 CRUD/删除/定位、窄窗目录抽屉、多窗口协调和隔离 P1/P2 桌面测试等底座。第二阶段分支 `codex/plainroot-stage-2` 的 T18～T34 又建立单一 `DocumentSession`、两种编辑 adapter、自动/手动保存与窗口结算、恢复/冲突/另存、受控图片链路、目录移动风险提示、原生编辑菜单、固定主操作工具栏和单一保存/路径状态栏；执行计划见 `agent-works/markdown-editor-desktop/stage-2-markdown-editing/plan.md`。
...
- 当前自动化测试在既有底座、编辑器、恢复/冲突/资源与菜单链路上共 180 个 Rust 测试通过（另 1 项手动性能探针忽略）、3 个工作区路径测试、18 个前端树状态测试、4 个永久删除反馈测试、176 个 React UI/状态/编辑器测试、1 个 fixture 测试、4 个许可证策略测试和 9 个桌面 E2E。`pnpm test` 统一执行 Node、Vitest 与无桌面 feature 的 Rust 服务门禁；`test:editor`、`test:roundtrip`、`test:rust` 可单独定位。GitHub Actions run `30082725332` 已在 macOS/Windows 完整通过当前 9 条桌面门禁、生产构建和 artifact 上传。许可证扫描为 727 个 Node 包、508 个 Rust 包、0 个阻断项。Windows 原生系统 UI 仍未人工验证。
...
- 修改代码或文档前检查最近一次远端同步时间；超过 2 小时先同步并解决冲突。最近一次远端同步：2026-07-24 17:42 CST（当前分支跟踪 `origin/codex/plainroot-stage-2`；提交 `914ad8413b30569ab1c704dc1a55f15d3ed78c59` 已推送并通过双平台 CI）。
